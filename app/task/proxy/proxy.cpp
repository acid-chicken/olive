...
ProxyTask::ProxyTask(VideoStreamPtr stream, int divider) : stream_(stream), divider_(divider) {
  if (divider_ == 1) {
    SetTitle(tr("Generating full resolution proxy %1:%2")
                 .arg(stream_->footage()->filename(), QString::number(stream_->index())));
  } else {
    SetTitle(tr("Generating 1/%1 resolution proxy %2:%3")
                 .arg(QString::number(divider), stream_->footage()->filename(), QString::number(stream_->index())));
  }
}
