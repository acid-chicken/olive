...
TaskManager::TaskManager() : active_thread_count_(0) {
...
TaskManager::~TaskManager() {
  ...
  foreach (const ThreadContainer& tc, threads_) { tc.thread->quit(); }
  ...
  foreach (const TaskContainer& task_info, tasks_) { delete task_info.task; }
}
...
Task* TaskManager::GetFirstTask() const { return tasks_.first().task; }
...
void TaskManager::TaskSucceeded() { SetTaskStatus(static_cast<Task*>(sender()), kFinished); }

void TaskManager::TaskFailed() { SetTaskStatus(static_cast<Task*>(sender()), kError); }
