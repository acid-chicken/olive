...
AudioOutputManager::AudioOutputManager(QObject* parent) : QObject(parent), output_(nullptr), push_device_(nullptr) {}

AudioOutputManager::~AudioOutputManager() { Close(); }
...
void AudioOutputManager::SetParameters(AudioRenderingParams params) { device_proxy_.SetParameters(params); }
...
void AudioOutputManager::PushMoreSamples() {
  ...
  qint64 write_count = push_device_->write(read_ptr, push_samples_.size() - push_sample_index_);
...
void AudioOutputManager::OutputStateChanged(QAudio::State state) { qDebug() << state << output_->error(); }
