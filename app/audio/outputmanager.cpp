...
AudioOutputManager::AudioOutputManager(QObject *parent)
    : QObject(parent), output_(nullptr), push_device_(nullptr), enable_sending_samples_(false) {
  connect(&device_proxy_, &AudioOutputDeviceProxy::ProcessedAverages, this, &AudioOutputManager::SentSamples);
}

bool AudioOutputManager::OutputIsSet() { return (output_.get()); }
...
void AudioOutputManager::SetParameters(const AudioRenderingParams &params) { device_proxy_.SetParameters(params); }
...
void AudioOutputManager::PushMoreSamples() {
  ...
  qint64 write_count = push_device_->write(read_ptr, pushed_samples_.size() - pushed_sample_index_);
