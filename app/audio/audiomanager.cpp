...
bool AudioManager::IsRefreshingOutputs() { return output_watcher_.isRunning(); }

bool AudioManager::IsRefreshingInputs() { return input_watcher_.isRunning(); }
...
void AudioManager::StartOutput(const QString &filename, qint64 offset, int playback_speed) {
  QMetaObject::invokeMethod(&output_manager_, "PullFromDevice", Qt::QueuedConnection, Q_ARG(const QString &, filename),
                            Q_ARG(qint64, offset), Q_ARG(int, playback_speed));

  emit OutputDeviceStarted(filename, offset, playback_speed);
}

void AudioManager::StopOutput() {
  QMetaObject::invokeMethod(&output_manager_, "ResetToPushMode", Qt::QueuedConnection);
...
void AudioManager::SetOutputDevice(const QAudioDeviceInfo &info) {
  ...
  if (output_params_.is_valid()) {
    ...
    if (info.isFormatSupported(format)) {
      QMetaObject::invokeMethod(&output_manager_, "SetOutputDevice", Qt::QueuedConnection,
                                Q_ARG(const QAudioDeviceInfo &, info), Q_ARG(const QAudioFormat &, format));
      output_is_set_ = true;
    } else {
...
void AudioManager::SetOutputParams(const AudioRenderingParams &params) {
  if (output_params_ != params) {
    ...
    QMetaObject::invokeMethod(&output_manager_, "SetParameters", Qt::QueuedConnection,
...
AudioManager::AudioManager() : output_is_set_(false), input_(nullptr), input_file_(nullptr) {
  RefreshDevices();
  ...
  connect(&output_watcher_, &QFutureWatcher<QList<QAudioDeviceInfo> >::finished, this,
          &AudioManager::OutputDevicesRefreshed);
  connect(&input_watcher_, &QFutureWatcher<QList<QAudioDeviceInfo> >::finished, this,
          &AudioManager::InputDevicesRefreshed);
}
