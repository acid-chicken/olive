...
void MainWindow::LoadLayout(QXmlStreamReader* reader, XMLNodeData& xml_data) {
  QMetaObject::invokeMethod(this, "LoadLayoutInternal", Qt::BlockingQueuedConnection, Q_ARG(QXmlStreamReader*, reader),
...
ScopePanel* MainWindow::AppendScopePanel() { return AppendFloatingPanelInternal<ScopePanel>(scope_panels_); }
...
void MainWindow::TimelineCloseRequested() { RemoveTimelinePanel(static_cast<TimelinePanel*>(sender())); }
...
TimelinePanel* MainWindow::AppendTimelinePanel() {
  ...
  connect(sequence_viewer_panel_->video_renderer(), &VideoRenderBackend::CachedTimeReady, panel->ruler(),
          &TimeRuler::CacheTimeReady);
  connect(sequence_viewer_panel_->video_renderer(), &VideoRenderBackend::RangeInvalidated, panel->ruler(),
          &TimeRuler::CacheInvalidatedRange);
...
void MainWindow::SetDefaultLayout() {
  ...
  resizeDocks({node_panel_, param_panel_, sequence_viewer_panel_}, {width() / 3, width() / 3, width() / 3},
              Qt::Horizontal);

  resizeDocks({project_panels_.first(), tool_panel_, timeline_panels_.first(), audio_monitor_panel_},
              {width() / 4, 1, width(), 1}, Qt::Horizontal);

  resizeDocks({node_panel_, project_panels_.first()}, {height() / 2, height() / 2}, Qt::Vertical);
}
