...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
  ...
  connect(viewer_panel_->video_renderer(), &VideoRenderBackend::CachedTimeReady, timeline_panel_->ruler(),
          &TimeRuler::CacheTimeReady);
  connect(viewer_panel_->video_renderer(), &VideoRenderBackend::RangeInvalidated, timeline_panel_->ruler(),
...
void MainWindow::closeEvent(QCloseEvent* e) {
  ...
  foreach (ViewerPanel* viewer, viewers) { viewer->ConnectViewerNode(nullptr); }
...
void MainWindow::UpdateTitle() {
  setWindowTitle(QStringLiteral("%1 %2 - %3")
                     .arg(QApplication::applicationName(), QApplication::applicationVersion(), tr("<untitled>")));
}
...
void MainWindow::SetDefaultLayout() {
  ...
  resizeDocks({node_panel_, param_panel_, viewer_panel_}, {width() / 3, width() / 3, width() / 3}, Qt::Horizontal);

  resizeDocks({project_panel_, tool_panel_, timeline_panel_, audio_monitor_panel_}, {width() / 4, 1, width(), 1},
              Qt::Horizontal);

  resizeDocks({node_panel_, project_panel_}, {height() / 2, height() / 2}, Qt::Vertical);
}
