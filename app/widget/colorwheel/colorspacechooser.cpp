...
ColorSpaceChooser::ColorSpaceChooser(ColorManager* color_manager, bool enable_input_field, bool enable_display_fields,
  ...
  if (enable_input_field) {
    ...
    foreach (const QString& s, input_spaces) { input_combobox_->addItem(s); }
  ...
  if (enable_display_fields) {
    {
      ...
      foreach (const QString& s, display_spaces) { display_combobox_->addItem(s); }
...

    {
      ...
      foreach (const QString& s, looks) { look_combobox_->addItem(s, s); }
...
void ColorSpaceChooser::UpdateViews(const QString& display) {
  ...
  foreach (const QString& s, views) { view_combobox_->addItem(s); }
