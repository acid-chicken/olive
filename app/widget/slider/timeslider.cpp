...
TimeSlider::TimeSlider(QWidget *parent) : IntegerSlider(parent) {
  SetMinimum(0);
...
QString TimeSlider::ValueToString(const QVariant &v) {
  ...
  return Timecode::timestamp_to_timecode(v.toLongLong(), timebase_, Core::instance()->GetTimecodeDisplay());
}
...
void TimeSlider::TimecodeDisplayChanged() { UpdateLabel(Value()); }
