...
void ProjectExplorer::Edit(Item* item) { CurrentView()->edit(model_.CreateIndexFromItem(item)); }
...
void ProjectExplorer::ItemDoubleClickedSlot(const QModelIndex& index) {
  ...
  if (i->CanHaveChildren() && (view_type() == ProjectToolbar::ListView || view_type() == ProjectToolbar::IconView)) {
    ...
    if (context_menu_item_->type() == Item::kFolder) {
      ...
      if (f->HasStreamsOfType(Stream::kVideo)) {
        ...
        VideoStreamPtr video_stream =
            std::static_pointer_cast<VideoStream>(f->get_first_stream_of_type(Stream::kVideo));
...
QModelIndex ProjectExplorer::get_root_index() const { return tree_view_->rootIndex(); }
