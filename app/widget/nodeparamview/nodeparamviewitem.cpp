...
NodeParamViewItem::NodeParamViewItem(Node* node, QWidget* parent) : QWidget(parent), node_(node) {
...
void NodeParamViewItem::SetTimeTarget(Node* target) { body_->SetTimeTarget(target); }
...
Node* NodeParamViewItem::GetNode() const { return node_; }

void NodeParamViewItem::SignalAllKeyframes() { body_->SignalAllKeyframes(); }
...
NodeParamViewItemTitleBar::NodeParamViewItemTitleBar(QWidget* parent) : QWidget(parent) {}
...
NodeParamViewItemBody::NodeParamViewItemBody(const QVector<NodeInput*>& inputs, QWidget* parent) : QWidget(parent) {
  int row_count = 0;
  ...
  foreach (NodeInput* input, inputs) {
    ...
    connect(ui_objects.main_label, &ClickableLabel::MouseDoubleClicked, this,
            &NodeParamViewItemBody::LabelDoubleClicked);
    ...
    if (input->IsConnectable()) {
      ...
      connect(ui_objects.connected_label, &NodeParamViewConnectedLabel::ConnectionClicked, this,
              &NodeParamViewItemBody::ConnectionClicked);
    ...
    if (input->is_keyframable()) {
      ...
      connect(ui_objects.key_control, &NodeParamViewKeyframeControl::RequestSetTime, this,
              &NodeParamViewItemBody::RequestSetTime);
...
void NodeParamViewItemBody::SetTimeTarget(Node* target) {
  ...
  foreach (NodeParamViewItemBody* sb, sub_bodies_) { sb->SetTimeTarget(target); }
}
...
void NodeParamViewItemBody::SetTime(const rational& time) {
  ...
  foreach (NodeParamViewItemBody* sb, sub_bodies_) { sb->SetTime(time); }
}
...
void NodeParamViewItemBody::Retranslate() {
  ...
  foreach (NodeParamViewItemBody* sb, sub_bodies_) { sb->Retranslate(); }
}
...
void NodeParamViewItemBody::SignalAllKeyframes() {
  ...
  for (i = input_ui_map_.begin(); i != input_ui_map_.end(); i++) {
    ...
    foreach (const NodeInput::KeyframeTrack& track, input->keyframe_tracks()) {
      foreach (NodeKeyframePtr key, track) { InputAddedKeyframeInternal(input, key); }
    }
  }

  foreach (NodeParamViewItemBody* sb, sub_bodies_) { sb->SignalAllKeyframes(); }
}

void NodeParamViewItemBody::EdgeChanged() { UpdateUIForEdgeConnection(static_cast<NodeInput*>(sender())); }
...
void NodeParamViewItemBody::UpdateUIForEdgeConnection(NodeInput* input) {
  ...
  foreach (QWidget* w, ui_objects.widget_bridge->widgets()) { w->setVisible(!input->IsConnected()); }
...
NodeParamViewItemBody::InputUI::InputUI()
    : main_label(nullptr), widget_bridge(nullptr), connected_label(nullptr), key_control(nullptr) {}
