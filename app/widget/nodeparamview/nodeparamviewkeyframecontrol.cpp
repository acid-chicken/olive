...
NodeInput* NodeParamViewKeyframeControl::GetConnectedInput() const { return input_; }
...
void NodeParamViewKeyframeControl::ToggleKeyframe(bool e) {
  ...
  if (e && keys.isEmpty()) {
    ...
    for (int i = 0; i < input_->get_number_of_keyframe_tracks(); i++) {
      NodeKeyframePtr key = NodeKeyframe::Create(node_time, input_->get_value_at_time_for_track(node_time, i),
                                                 input_->get_best_keyframe_type_for_time(node_time, i), i);
    ...
    foreach (NodeKeyframePtr key, keys) {
      ...
      if (input_->keyframe_tracks().at(key->track()).size() == 1) {
        ...
        new NodeParamSetStandardValueCommand(input_, key->track(),
                                             input_->get_value_at_time_for_track(node_time, key->track()), command);
      }
    }
  }
...
void NodeParamViewKeyframeControl::KeyframeEnableChanged(bool e) {
  ...
  if (e) {
    ...
    for (int i = 0; i < key_vals.size(); i++) {
      NodeKeyframePtr key =
          NodeKeyframe::Create(GetCurrentTimeAsNodeTime(), key_vals.at(i), NodeKeyframe::kDefaultType, i);
