...
const QList<QWidget*>& NodeParamViewWidgetBridge::widgets() { return widgets_; }
  ...
  if (input_->is_keyframing()) {
    ...
    if (existing_key) {
      ...
      NodeKeyframePtr new_key =
          NodeKeyframe::Create(time_, value, input_->get_best_keyframe_type_for_time(time_, track), track);
      ...
      if (input_->is_keyframing()) {
        ...
        if (drag_created_keyframe_) {
          dragging_keyframe_ = NodeKeyframe::Create(
              time_, value, input_->get_best_keyframe_type_for_time(time_, slider_track), slider_track);
