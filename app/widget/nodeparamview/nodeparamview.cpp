...
NodeParamView::NodeParamView(QWidget* parent) : QWidget(parent), last_scroll_val_(0) {
...
void NodeParamView::SetNodes(QList<Node*> nodes) {
  ...
  foreach (NodeParamViewItem* item, items_) { delete item; }
  items_.clear();
...
const double& NodeParamView::GetScale() const { return ruler_->scale(); }
