...
CurveView::CurveView(QWidget* parent) : KeyframeViewBase(parent) {
...
CurveView::~CurveView() {
  // Quick way to avoid segfault when QGraphicsScene::selectionChanged is emitted after other members have been
  // destroyed
  Clear();
}
...
void CurveView::Clear() {
  ...
  foreach (QGraphicsLineItem* line, lines_) { delete line; }
  lines_.clear();
}

void CurveView::SetTrackCount(int count) { track_count_ = count; }
      ...
      for (int i = 1; i < keys.size(); i++) {
        ...
        if (before->type() == NodeKeyframe::kHold) {
          ...
          keyframe_lines.append(
              QLineF(before_item->pos().x(), before_item->pos().y(), after_item->pos().x(), before_item->pos().y()));
          keyframe_lines.append(
              QLineF(after_item->pos().x(), before_item->pos().y(), after_item->pos().x(), after_item->pos().y()));
...
void CurveView::ScaleChangedEvent(const double& scale) {
  ...
  foreach (BezierControlPointItem* item, bezier_control_points_) { item->SetXScale(scale); }
}
...
qreal CurveView::GetItemYFromKeyframeValue(NodeKeyframe* key) { return -key->value().toDouble() * GetYScale(); }
...
void CurveView::SelectionChanged() {
  ...
  foreach (BezierControlPointItem* item, bezier_control_points_) { delete item; }
