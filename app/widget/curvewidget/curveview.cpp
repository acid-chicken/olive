...
CurveView::~CurveView() {
  // Quick way to avoid segfault when QGraphicsScene::selectionChanged is emitted after other memebers have been
  // destroyed
  Clear();
}
...
qreal CurveView::GetItemYFromKeyframeValue(NodeKeyframe* key) { return -key->value().toDouble() * y_scale_; }
