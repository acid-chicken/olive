...
CurveWidget::CurveWidget(QWidget* parent) : TimeBasedWidget(parent), input_(nullptr), bridge_(nullptr) {
  ...
  connect(key_control_, &NodeParamViewKeyframeControl::RequestSetTime, this,
          &CurveWidget::KeyControlRequestedTimeChanged);
...
NodeInput* CurveWidget::GetInput() const { return input_; }

void CurveWidget::SetInput(NodeInput* input) {
  if (bridge_) {
    foreach (QWidget* bridge_widget, bridge_->widgets()) { delete bridge_widget; }
    delete bridge_;
    bridge_ = nullptr;
  }
  ...
  if (input_) {
    ...
    foreach (const NodeInput::KeyframeTrack& track, input_->keyframe_tracks()) {
      foreach (NodeKeyframePtr key, track) { view_->AddKeyframe(key); }
    }
  }
