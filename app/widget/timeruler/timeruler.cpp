...
const double &TimeRuler::GetScale() { return scale_; }
...
const int64_t &TimeRuler::GetTime() { return time_; }
      ...
      if (this_long_unit != last_long_unit) {
        ...
        if (text_visible_) {
          ...
          QString timecode_str =
              Timecode::timestamp_to_timecode(ScreenToUnit(i), timebase_, Timecode::CurrentDisplay());
  ...
  if (show_cache_status_) {
    ...
    if (cache_screen_length > 0) {
      ...
      foreach (const TimeRange &range, dirty_cache_ranges_) {
        ...
        p.fillRect(qMax(0, range_left), cache_y, qMin(width(), range_right) - range_left, cache_status_height_,
                   Qt::red);
      }
    }
  }
...
void TimeRuler::mousePressEvent(QMouseEvent *event) { SeekToScreenPoint(event->pos().x()); }
...
double TimeRuler::ScreenToUnitFloat(int screen) { return (screen + scroll_) / scale_ / timebase_dbl_; }

int64_t TimeRuler::ScreenToUnit(int screen) { return qFloor(ScreenToUnitFloat(screen)); }

int TimeRuler::UnitToScreen(int64_t unit) {
  return qFloor(static_cast<double>(unit) * scale_ * timebase_dbl_) - scroll_;
}

int TimeRuler::TimeToScreen(const rational &time) { return qFloor(time.toDouble() * scale_) - scroll_; }
