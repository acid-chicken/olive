...
void TimelineWidget::PointerTool::MousePress(TimelineViewMouseEvent* event) {
  ...
  if (selectable_item && item->isSelected()) {
...
void TimelineWidget::PointerTool::SetMovementAllowed(bool allowed) { movement_allowed_ = allowed; }

void TimelineWidget::PointerTool::SetTrackMovementAllowed(bool allowed) { track_movement_allowed_ = allowed; }

void TimelineWidget::PointerTool::SetTrimmingAllowed(bool allowed) { trimming_allowed_ = allowed; }
...
void TimelineWidget::PointerTool::MouseReleaseInternal(TimelineViewMouseEvent* event) {
  ...
  for (int i = 0; i < parent()->ghost_items_.size(); i++) {
    ...
    if (Timeline::IsATrimMode(ghost->mode())) {
      ...
      new NodeAddCommand(static_cast<NodeGraph*>(b->parent()), copy, command);
      ...
      if (transition->connected_in_block() && transition->connected_out_block()) {
        new BlockSetMediaInCommand(transition, transition->media_in() + ghost->InAdjustment(), command);
      }
    }
  ...
  if (command->childCount() > 0) {
    foreach (const TrackReference& t, tracks_affected) {
      new TrackCleanGapsCommand(parent()->GetConnectedNode()->track_list(t.type()), t.index(), command);
    }
  }
...
rational TimelineWidget::PointerTool::FrameValidateInternal(rational time_movement,
...
void TimelineWidget::PointerTool::ProcessDrag(const TimelineCoordinate& mouse_pos) {
  ...
  QString tooltip_text =
      Timecode::timestamp_to_timecode(earliest_timestamp, parent()->timebase(), Timecode::CurrentDisplay(), true);
...
void TimelineWidget::PointerTool::InitiateGhosts(TimelineViewBlockItem* clicked_item, Timeline::MovementMode trim_mode,
  ...
  foreach (TimelineViewBlockItem* clip_item, clips) {
    ...
    if (clip_item->block()->type() == Block::kGap && !Timeline::IsATrimMode(trim_mode)) {
      continue;
    }

    if (clip_item != clicked_item && (Timeline::IsATrimMode(trim_mode))) {
...
TimelineViewGhostItem* TimelineWidget::PointerTool::AddGhostFromBlock(Block* block, const TrackReference& track,
                                                                      Timeline::MovementMode mode) {
  TimelineViewGhostItem* ghost =
      TimelineViewGhostItem::FromBlock(block, track, parent()->GetTrackY(track), parent()->GetTrackHeight(track));
...
TimelineViewGhostItem* TimelineWidget::PointerTool::AddGhostFromNull(const rational& in, const rational& out,
...
rational GetEarliestPointForClip(Block* block) { return qMax(rational(0), block->in() - block->media_in()); }
