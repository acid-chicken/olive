...
TimelineWidget::RippleTool::RippleTool(TimelineWidget* parent) : PointerTool(parent) { SetMovementAllowed(false); }
  ...
  foreach (TimelineViewGhostItem* ghost, parent()->ghost_items_) {
    ...
    if (b == nullptr) {
      ...
      if (ghost->AdjustedLength() > 0) {
        ...
        new TrackInsertBlockAfterCommand(parent()->GetTrackFromReference(ghost->Track()), gap, block_to_append_gap_to,
                                         command);
      }
    } else {
...
rational TimelineWidget::RippleTool::FrameValidateInternal(rational time_movement,
...
void TimelineWidget::RippleTool::InitiateGhosts(TimelineViewBlockItem* clicked_item, Timeline::MovementMode trim_mode,
