...
void TimelineWidget::ImportTool::DragEnter(TimelineViewMouseEvent* event) {
  ...
  if (mime_formats.contains("application/x-oliveprojectitemdata")) {
    ...
    PrepGhosts(drag_start_.GetFrame() - parent()->SceneToTime(import_pre_buffer_), drag_start_.GetTrack().index());
...
void TimelineWidget::ImportTool::FootageToGhosts(rational ghost_start, const QList<Footage*>& footage_list,
  ...
  foreach (Footage* footage, footage_list) {
    ...
    foreach (StreamPtr stream, footage->streams()) {
      ...
      if (stream->type() == Stream::kImage) {
        ...
        stream_duration =
            qCeil(static_cast<double>(stream_duration) * stream->timebase().toDouble() / dest_tb.toDouble());

        // Convert to rational time
        footage_duration = rational(dest_tb.numerator() * stream_duration, dest_tb.denominator());
      }
...
void TimelineWidget::ImportTool::PrepGhosts(const rational& frame, const int& track_index) {
  if (parent()->GetConnectedNode()) {
    FootageToGhosts(frame, dragged_footage_, parent()->timebase(), track_index);
  }
}
...
void TimelineWidget::ImportTool::DropGhosts(bool insert) {
  ...
  if (parent()->GetConnectedNode()) {
    ...
    DropWithoutSequenceBehavior behavior =
        static_cast<DropWithoutSequenceBehavior>(Config::Current()["DropWithoutSequenceBehavior"].toInt());
    ...
    if (behavior == kDWSAsk) {
      ...
      QPushButton* auto_params_btn =
          mbox.addButton(tr("Automatically Detect Parameters From Footage"), QMessageBox::YesRole);
      ...
      if (active_project) {
        ...
        if (sequence_is_valid) {
          ...
          new ProjectViewModel::AddItemCommand(Core::instance()->GetActiveProjectModel(),
                                               Core::instance()->GetSelectedFolderInActiveProject(), new_sequence,
                                               command);
