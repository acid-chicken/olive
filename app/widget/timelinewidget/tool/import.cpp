...
void TimelineWidget::ImportTool::FootageToGhosts(rational ghost_start, const QList<DraggedFootage>& footage_list,
  ...
  foreach (const DraggedFootage& footage, footage_list) {
    ...
    foreach (StreamPtr stream, footage.footage()->streams()) {
      ...
      if (track_type == Timeline::kTrackTypeNone || !(cached_enabled_streams & 0x1)) {
        continue;
      }
    ...
    if (behavior != kDWSDisable) {
      ...
      if (active_project) {
        ...
        if (behavior == kDWSAuto) {
          ...
          foreach (const DraggedFootage& df, dragged_footage_) { footage_only.append(df.footage()); }
