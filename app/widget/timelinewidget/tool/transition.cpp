...
TimelineWidget::TransitionTool::TransitionTool(TimelineWidget* parent) : AddTool(parent) {}
...
void TimelineWidget::TransitionTool::MousePress(TimelineViewMouseEvent* event) {
  ...
  if (cursor_frame < halfway_point) {
    ...
    if (cursor_frame < tenth_point && block_at_time->previous() && block_at_time->previous()->type() == Block::kClip) {
    ...
    if (cursor_frame > block_at_time->length() - tenth_point && block_at_time->next() &&
  ...
  if (other_block) ghost_->setData(TimelineViewGhostItem::kReferenceBlock, Node::PtrToValue(other_block));
  ...
  if (ghost_) {
    if (!ghost_->AdjustedLength().isNull()) {
      TransitionBlock* transition =
          static_cast<TransitionBlock*>(NodeFactory::CreateFromID("org.olivevideoeditor.Olive.crossdissolve"));

      QUndoCommand* command = new QUndoCommand();

      // Place transition in place
      new NodeAddCommand(static_cast<NodeGraph*>(parent()->GetConnectedNode()->parent()), transition, command);

      new TrackPlaceBlockCommand(parent()->GetConnectedNode()->track_list(track.type()), track.index(), transition,
      ...
      if (dual_transition_) {
        ...
        new NodeEdgeAddCommand(out_block->output(), transition->out_block_input(), command);

        new NodeEdgeAddCommand(in_block->output(), transition->in_block_input(), command);
      } else {
        ...
        // Connect block to transition
        new NodeEdgeAddCommand(block_to_transition->output(), transition_input_to_connect, command);
      }
