...
TimelineWidget::RazorTool::RazorTool(TimelineWidget* parent) : Tool(parent) {}
...
void TimelineWidget::RazorTool::MouseRelease(TimelineViewMouseEvent* event) {
  ...
  foreach (const TrackReference& track_ref, split_tracks_) {
    ...
    if (block_at_time && block_at_time->type() == Block::kClip && !blocks_to_split.contains(block_at_time)) {
