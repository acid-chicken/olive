...
TimelineWidget::Tool::Tool(TimelineWidget* parent) : dragging_(false), parent_(parent) {}

TimelineWidget::Tool::~Tool() {}

TimelineWidget* TimelineWidget::Tool::parent() { return parent_; }
...
TimelineViewBlockItem* TimelineWidget::Tool::GetItemAtScenePos(const TimelineCoordinate& coord) {
  ...
  while (iterator.hasNext()) {
    ...
    if (b->in() <= coord.GetFrame() && b->out() > coord.GetFrame() && item->Track() == coord.GetTrack()) {
      return item;
    }
  }
...
void AttemptSnap(const QList<double>& proposed_pts, double compare_point, const QList<rational>& start_times,
                 rational compare_time, rational* movement, double* diff) {
  ...
  for (int i = 0; i < proposed_pts.size(); i++) {
    ...
    if (InRange(proposed_pts.at(i), compare_point, kSnapRange)) {
      ...
      if (this_diff < *diff && start_times.at(i) + *movement >= 0) {
...
bool TimelineWidget::Tool::SnapPoint(QList<rational> start_times, rational* movement, int snap_points) {
  ...
  foreach (rational s, start_times) { proposed_pts.append((s + *movement).toDouble() * parent()->GetScale()); }

  if (snap_points & kSnapToPlayhead) {
    rational playhead_abs_time =
        rational(parent()->GetTimestamp() * parent()->timebase().numerator(), parent()->timebase().denominator());
...
void TimelineWidget::Tool::GetGhostData(const QVector<TimelineViewGhostItem*>& ghosts, rational* earliest_point,
...
void TimelineWidget::Tool::InsertGapsAtGhostDestination(const QVector<TimelineViewGhostItem*>& ghosts,
