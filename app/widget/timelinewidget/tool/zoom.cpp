...
TimelineWidget::ZoomTool::ZoomTool(TimelineWidget *parent) : Tool(parent) {}

void TimelineWidget::ZoomTool::MousePress(TimelineViewMouseEvent *event) { Q_UNUSED(event) }
...
void TimelineWidget::ZoomTool::MouseRelease(TimelineViewMouseEvent *event) {
  if (dragging_) {
    ...
    double new_scale =
        qMin(TimelineViewBase::kMaximumScale, static_cast<double>(reference_view->viewport()->width()) / scene_width);
    ...
    parent()->deferred_scroll_value_ =
        qMax(0, qRound(frame_x - parent()->views_.first()->view()->viewport()->width() / 2));
  }
