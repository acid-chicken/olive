...
class TimelineWidget : public TimeBasedWidget, public NodeCopyPasteWidget {
 ...
 private:
  class DraggedFootage {
   public:
    DraggedFootage(Footage* f, quint64 streams) : footage_(f), streams_(streams) {}

    Footage* footage() const { return footage_; }

    const quint64& streams() const { return streams_; }
  ...
  class ImportTool : public Tool {
   ...
   private:
    void FootageToGhosts(rational ghost_start, const QList<DraggedFootage>& footage, const rational& dest_tb,
  ...
  void DeleteSelectedInternal(const QList<Block*>& blocks, bool transition_aware, bool remove_from_graph,
