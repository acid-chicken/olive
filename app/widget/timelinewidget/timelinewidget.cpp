...
TimelineWidget::TimelineWidget(QWidget* parent)
  ...
  foreach (TimelineAndTrackView* tview, views_) {
    ...
    foreach (TimelineAndTrackView* other_tview, views_) {
      ...
      if (view != other_view) {
        connect(view->horizontalScrollBar(), &QScrollBar::valueChanged, other_view->horizontalScrollBar(),
                &QScrollBar::setValue);
      }
    }
  }
...
void TimelineWidget::ScaleChangedEvent(const double& scale) {
  ...
  foreach (TimelineViewGhostItem* ghost, ghost_items_) { ghost->SetScale(scale); }

  foreach (TimelineAndTrackView* view, views_) { view->view()->SetScale(scale); }
}
...
void TimelineWidget::ConnectNodeInternal(ViewerOutput* n) {
  ...
  for (int i = 0; i < views_.size(); i++) {
    ...
    foreach (TrackOutput* track, n->track_list(track_type)->GetTracks()) { AddTrack(track, track_type); }
  }
}
...
QList<TimelineWidget::DraggedFootage> TimelineWidget::FootageToDraggedFootage(QList<Footage*> footage) {
  ...
  foreach (Footage* f, footage) { df.append(FootageToDraggedFootage(f)); }
...
void TimelineWidget::SelectAll() {
  foreach (TimelineAndTrackView* view, views_) { view->view()->SelectAll(); }
}

void TimelineWidget::DeselectAll() {
  foreach (TimelineAndTrackView* view, views_) { view->view()->DeselectAll(); }
}

void TimelineWidget::RippleToIn() { RippleEditTo(Timeline::kTrimIn, false); }

void TimelineWidget::RippleToOut() { RippleEditTo(Timeline::kTrimOut, false); }

void TimelineWidget::EditToIn() { RippleEditTo(Timeline::kTrimIn, true); }

void TimelineWidget::EditToOut() { RippleEditTo(Timeline::kTrimOut, true); }
...
void TimelineWidget::DeleteSelectedInternal(const QList<Block*>& blocks, bool transition_aware, bool remove_from_graph,
  ...
  foreach (Block* b, blocks) {
    ...
    if (transition_aware && b->type() == Block::kTransition) {
      ...
      new TrackRippleRemoveBlockCommand(original_track, transition, command);
      ...
      if (transition->connected_out_block()) {
        new BlockResizeCommand(transition->connected_out_block(),
                               transition->connected_out_block()->length() + transition->out_offset(), command);
      }
    } else {
      ...
      new TrackReplaceBlockCommand(original_track, b, gap, command);
    }
...
void TimelineWidget::DeleteSelected(bool ripple) {
  ...
  foreach (const TrackReference& track, tracks_affected) {
    new TrackCleanGapsCommand(GetConnectedNode()->track_list(track.type()), track.index(), command);
  }
  ...
  if (ripple) {
    ...
    foreach (Block* b, blocks_to_delete) { range_list.InsertTimeRange(TimeRange(b->in(), b->out())); }
...
void TimelineWidget::IncreaseTrackHeight() {
  ...
  foreach (TrackOutput* t, all_tracks) { t->SetTrackHeight(t->GetTrackHeight() + t->GetTrackHeightIncrement()); }
}
...
void TimelineWidget::Paste(bool insert) {
  ...
  QList<Node*> pasted =
      PasteNodesFromClipboard(static_cast<Sequence*>(GetConnectedNode()->parent()), command, &paste_data);
  ...
  foreach (const BlockPasteData& bpd, paste_data) {
    foreach (Node* n, pasted) {
      if (n->property("xml_ptr") == bpd.ptr) {
        qDebug() << "Placing" << n;
        new TrackPlaceBlockCommand(GetConnectedNode()->track_list(bpd.track_type), bpd.track_index,
                                   static_cast<Block*>(n), paste_start + bpd.in, command);
        break;
      }
    }
  }
...
void TimelineWidget::DeleteInToOut(bool ripple) {
  if (!GetConnectedNode() || !GetConnectedTimelinePoints() || !GetConnectedTimelinePoints()->workarea()->enabled()) {
    return;
  }
  ...
  foreach (TrackOutput* track, GetConnectedNode()->Tracks()) {
    if (!track->IsLocked()) {
      if (ripple) {
        new TrackRippleRemoveAreaCommand(track, GetConnectedTimelinePoints()->workarea()->in(),
                                         GetConnectedTimelinePoints()->workarea()->out(), command);
      } else {
        GapBlock* gap = new GapBlock();
        ...
        new TrackPlaceBlockCommand(GetConnectedNode()->track_list(track->track_type()), track->Index(), gap,
                                   GetConnectedTimelinePoints()->workarea()->in(), command);
      }
    }
  }
...
void TimelineWidget::ToggleSelectedEnabled() {
  ...
  foreach (TimelineViewBlockItem* i, items) {
    new BlockEnableDisableCommand(i->block(), !i->block()->is_enabled(), command);
  }
...
void TimelineWidget::RippleEditTo(Timeline::MovementMode mode, bool insert_gaps) {
  ...
  foreach (TrackOutput* track, GetConnectedNode()->Tracks()) {
    ...
    TrackRippleRemoveAreaCommand* ripple_command =
        new TrackRippleRemoveAreaCommand(track, in_ripple, out_ripple, command);
...
void TimelineWidget::InsertGapsAt(const rational& earliest_point, const rational& insert_length,
  ...
  foreach (Block* gap, gaps_to_extend) { new BlockResizeCommand(gap, gap->length() + insert_length, command); }
...
void TimelineWidget::ClearGhosts() {
  if (!ghost_items_.isEmpty()) {
    foreach (TimelineViewGhostItem* ghost, ghost_items_) { delete ghost; }

    ghost_items_.clear();
  }
}

bool TimelineWidget::HasGhosts() { return !ghost_items_.isEmpty(); }

TimelineWidget::Tool* TimelineWidget::GetActiveTool() { return tools_.at(Core::instance()->tool()); }
...
void TimelineWidget::ViewDragEntered(TimelineViewMouseEvent* event) { import_tool_->DragEnter(event); }

void TimelineWidget::ViewDragMoved(TimelineViewMouseEvent* event) { import_tool_->DragMove(event); }

void TimelineWidget::ViewDragLeft(QDragLeaveEvent* event) { import_tool_->DragLeave(event); }

void TimelineWidget::ViewDragDropped(TimelineViewMouseEvent* event) { import_tool_->DragDrop(event); }
...
void TimelineWidget::RemoveBlock(Block* block) { delete block_items_.take(block); }

void TimelineWidget::AddTrack(TrackOutput* track, Timeline::TrackType type) {
  foreach (Block* b, track->Blocks()) { AddBlock(b, TrackReference(type, track->Index())); }
...
void TimelineWidget::RemoveTrack(TrackOutput* track) {
  ...
  foreach (Block* b, track->Blocks()) { RemoveBlock(b); }
}
...
void TimelineWidget::ViewSelectionChanged() {
  ...
  foreach (TimelineViewBlockItem* item, selected_items) { selected_blocks.append(item->block()); }
...
void TimelineWidget::SetViewTimestamp(const int64_t& ts) {
  for (int i = 0; i < views_.size(); i++) {
    ...
    if (use_audio_time_units_ && i == Timeline::kTrackTypeAudio) {
      view->view()->SetTime(
          Timecode::rescale_timestamp(ts, timebase(), GetConnectedNode()->audio_params().time_base()));
    } else {
...
void TimelineWidget::ViewTimestampChanged(int64_t ts) {
  if (use_audio_time_units_ && sender() == views_.at(Timeline::kTrackTypeAudio)) {
    ts = Timecode::rescale_timestamp(ts, GetConnectedNode()->audio_params().time_base(), timebase());
  }
...
void TimelineWidget::MoveRubberBandSelect(bool enable_selecting, bool select_links) {
  ...
  foreach (TimelineAndTrackView* tview, views_) {
    ...
    QRect mapped_rect(view->viewport()->mapFromGlobal(drag_origin_), view->viewport()->mapFromGlobal(rubberband_now));
  ...
  foreach (QGraphicsItem* item, rubberband_now_selected_) { item->setSelected(false); }
