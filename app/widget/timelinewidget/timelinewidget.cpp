...
void TimelineWidget::ConnectNodeInternal(ViewerOutput* n) {
  ...
  for (int i = 0; i < views_.size(); i++) {
    ...
    foreach (TrackOutput* track, n->track_list(track_type)->Tracks()) { AddTrack(track, track_type); }
  }
}
...
void TimelineWidget::UpdateTimelineLength(const rational& length) {
  foreach (TimelineAndTrackView* view, views_) { view->view()->SetEndTime(length); }
