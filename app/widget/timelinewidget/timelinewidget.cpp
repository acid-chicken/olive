...
TimelineWidget::TimelineWidget(QWidget* parent)
  ...
  connect(views_.first()->view()->horizontalScrollBar(), SIGNAL(rangeChanged(int, int)), horizontal_scroll_,
  ...
  foreach (TimelineAndTrackView* tview, views_) {
    ...
    connect(view, SIGNAL(MouseReleased(TimelineViewMouseEvent*)), this,
            SLOT(ViewMouseReleased(TimelineViewMouseEvent*)));
    connect(view, SIGNAL(MouseDoubleClicked(TimelineViewMouseEvent*)), this,
            SLOT(ViewMouseDoubleClicked(TimelineViewMouseEvent*)));
    ...
    foreach (TimelineAndTrackView* other_tview, views_) {
      ...
      if (view != other_view) {
        connect(view->horizontalScrollBar(), SIGNAL(valueChanged(int)), other_view->horizontalScrollBar(),
                SLOT(setValue(int)));
      }
    }
  }
...
TimelineWidget::~TimelineWidget() { qDeleteAll(tools_); }
...
void TimelineWidget::SetTimebase(const rational& timebase) {
  ...
  foreach (TimelineAndTrackView* view, views_) { view->view()->SetTimebase(timebase); }
}
...
void TimelineWidget::SetTime(int64_t timestamp) {
  ...
  foreach (TimelineAndTrackView* view, views_) { view->view()->SetTime(timestamp); }
...
void TimelineWidget::ConnectTimelineNode(ViewerOutput* node) {
  ...
  if (timeline_node_ != nullptr) {
    ...
    for (int i = 0; i < views_.size(); i++) {
      ...
      foreach (TrackOutput* track, timeline_node_->track_list(track_type)->Tracks()) { AddTrack(track, track_type); }
    }
  }
}

void TimelineWidget::DisconnectTimelineNode() { ConnectTimelineNode(nullptr); }

void TimelineWidget::ZoomIn() { SetScale(scale_ * 2); }

void TimelineWidget::ZoomOut() { SetScale(scale_ * 0.5); }
...
void TimelineWidget::DeleteSelected() {
  ...
  foreach (const TrackReference& track, tracks_affected) {
    new TrackCleanGapsCommand(timeline_node_->track_list(track.type()), track.index(), command);
  }
...
ViewerOutput* TimelineWidget::GetConnectedNode() const { return timeline_node_; }
...
void TimelineWidget::SetScale(double scale, bool center_on_playhead) {
  ...
  foreach (TimelineViewGhostItem* ghost, ghost_items_) { ghost->SetScale(scale_); }

  foreach (TimelineAndTrackView* view, views_) { view->view()->SetScale(scale_, center_on_playhead); }
}
...
void TimelineWidget::DeferredScrollAction() { horizontal_scroll_->setValue(deferred_scroll_value_); }
