...
void TimelineViewBlockItem::UpdateRect() {
  ...
  setToolTip(QCoreApplication::translate("TimelineViewBlockItem", "%1\n\nIn: %2\nOut: %3\nLength: %4")
                 .arg(block_->Name(),
                      ...
                      Timecode::time_to_timecode(block_->out() - block_->in(), timebase(),
...
void TimelineViewBlockItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *) {
  switch (block_->type()) {
    case Block::kClip: {
      ...
      QString wave_fn = QDir(QDir(Config::Current()["DiskCachePath"].toString()).filePath("waveform"))
      ...
      if (wave_file.open(QFile::ReadOnly)) {
        ...
        if (info.channels) {
          AudioWaveformView::DrawWaveform(painter, rect().toRect(), this->GetScale(),
                                          ...
                                          w.size() / sizeof(SampleSummer::Sum), info.channels);
        }
      }
