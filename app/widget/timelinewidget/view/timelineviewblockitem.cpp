...
TimelineViewBlockItem::TimelineViewBlockItem(Block *block, QGraphicsItem *parent)
    : TimelineViewRect(parent), block_(block) {
  setBrush(Qt::white);
  setCursor(Qt::DragMoveCursor);
  setFlag(QGraphicsItem::ItemIsSelectable,
          block_->type() == Block::kClip || block_->type() == Block::kGap || block_->type() == Block::kTransition);

  UpdateRect();
}

Block *TimelineViewBlockItem::block() const { return block_; }
...
void TimelineViewBlockItem::UpdateRect() {
  ...
  setToolTip(QCoreApplication::translate("TimelineViewBlockItem", "%1\n\nIn: %2\nOut: %3\nMedia In: %4")
                 .arg(block_->Name(), QString::number(block_->in().toDouble()),
                      QString::number(block_->out().toDouble()), QString::number(block_->media_in().toDouble())));
}
...
void TimelineViewBlockItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
  switch (block_->type()) {
    case Block::kClip: {
      ...
      QFile wave_file(QDir(QDir(Config::Current()["DiskCachePath"].toString()).filePath("waveform"))
                          .filePath(QString::number(reinterpret_cast<quintptr>(block_))));
      if (wave_file.open(QFile::ReadOnly)) {
        ...
        AudioWaveformView::DrawWaveform(painter, rect().toRect(), this->GetScale(),
                                        reinterpret_cast<const SampleSummer::Sum *>(w.constData()),
                                        w.size() / sizeof(SampleSummer::Sum), 2);
    ...
    case Block::kTransition: {
      ...
      if (t->connected_out_block() && t->connected_in_block()) {
        ...
        painter->drawLine(qRound(crossover_line), qRound(rect().top()), qRound(crossover_line),
                          qRound(rect().bottom()));
