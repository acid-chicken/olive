...
TimelinePoints *FootageViewerWidget::ConnectTimelinePoints() { return footage_; }

Project *FootageViewerWidget::GetTimelinePointsProject() { return footage_->project(); }
...
void FootageViewerWidget::StartFootageDragInternal(bool enable_video, bool enable_audio) {
  ...
  if (!enable_video || !enable_audio) {
    ...
    foreach (StreamPtr s, GetFootage()->streams()) {
      if ((s->type() == Stream::kVideo && !enable_video) || (s->type() == Stream::kAudio && !enable_audio)) {
