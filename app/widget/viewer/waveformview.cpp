...
WaveformView::WaveformView(QWidget* parent) : SeekableWidget(parent), backend_(nullptr) {
...
void WaveformView::SetBackend(AudioRenderBackend* backend) {
  if (backend_) {
    disconnect(backend_, &AudioRenderBackend::QueueComplete, this,
               static_cast<void (WaveformView::*)()>(&WaveformView::update));
...
void WaveformView::DrawWaveform(QPainter* painter, const QRect& rect, const double& scale,
  ...
  if (fs.open(QFile::ReadOnly)) {
    ...
    for (int x = 0; x < width() && !fs.atEnd(); x++) {
      ...
      for (int i = 0; i < params.channel_count(); i++) {
        ...
        p.drawLine(x, channel_mid + samples.at(i).min * static_cast<float>(channel_half_height), x,
...
void WaveformView::BackendParamsChanged() { SetTimebase(rational(1, backend_->params().sample_rate())); }
