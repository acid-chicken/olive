      ...
      for (int x = 0; x < width() && !fs.atEnd(); x++) {
        ...
        for (int i = 0; i < params.channel_count(); i++) {
          if (Config::Current()[QStringLiteral("RectifiedWaveforms")].toBool()) {
            ...
            wave_painter.drawLine(x, channel_mid + qRound(samples.at(i).min * static_cast<float>(channel_half_height)),
                                  x, channel_mid + qRound(samples.at(i).max * static_cast<float>(channel_half_height)));
          }
