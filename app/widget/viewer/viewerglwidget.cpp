...
ViewerGLWidget::ViewerGLWidget(QWidget *parent)
    : QOpenGLWidget(parent), texture_(0), ocio_lut_(0), color_manager_(nullptr) {
...
void ViewerGLWidget::SetupColorProcessor() {
  ...
  if (color_manager_) {
    ...
    color_service_ = ColorProcessor::Create(color_manager_->GetConfig(), OCIO::ROLE_SCENE_LINEAR, ocio_display_,
                                            ocio_view_, ocio_look_);

    // (Re)create pipeline from color processor
    pipeline_ = OpenGLShader::CreateOCIO(context(), ocio_lut_, color_service_->GetProcessor(), true);
  } else {
    color_service_ = nullptr;
    pipeline_ = nullptr;
  }
