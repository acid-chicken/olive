...
ViewerGLWidget::ViewerGLWidget(QWidget *parent)
    : QOpenGLWidget(parent), color_manager_(nullptr), has_image_(false), signal_cursor_color_(false) {
  setContextMenuPolicy(Qt::CustomContextMenu);
}

ViewerGLWidget::~ViewerGLWidget() { ContextCleanup(); }
...
void ViewerGLWidget::DisconnectColorManager() { ConnectColorManager(nullptr); }
  ...
  if (!fn.isEmpty()) {
    ...
    if (input) {
      PixelFormat::Format image_format =
          PixelFormat::OIIOFormatToOliveFormat(input->spec().format, input->spec().nchannels == kRGBAChannels);

      // Ensure the following texture operations are done in our context (in case we're in a separate window for
      // instance)
      makeCurrent();

      if (!texture_.IsCreated() || texture_.width() != input->spec().width ||
          texture_.height() != input->spec().height || texture_.format() != image_format) {
...
void ViewerGLWidget::SetImageFromLoadBuffer(Frame *in_buffer) {
  ...
  if (has_image_) {
    ...
    if (!texture_.IsCreated() || texture_.width() != in_buffer->width() || texture_.height() != in_buffer->height() ||
...
void ViewerGLWidget::SetOCIODisplay(const QString &display) {
  ...
  if (color_manager_ && !color_manager_->ListAvailableViews(ocio_display_).contains(ocio_view_)) {
...
ColorManager *ViewerGLWidget::color_manager() const { return color_manager_; }

const QString &ViewerGLWidget::ocio_display() const { return ocio_display_; }

const QString &ViewerGLWidget::ocio_view() const { return ocio_view_; }

const QString &ViewerGLWidget::ocio_look() const { return ocio_look_; }
  ...
  if (signal_cursor_color_) {
    ...
    if (has_image_) {
      QVector3D pixel_pos(static_cast<float>(event->x()) / static_cast<float>(width()) * 2.0f - 1.0f,
                          static_cast<float>(event->y()) / static_cast<float>(height()) * 2.0f - 1.0f, 0);
...
void ViewerGLWidget::SetupColorProcessor() {
  ...
  if (color_manager_) {
    ...
    try {
      color_service_ = OpenGLColorProcessor::Create(color_manager_->GetConfig(), OCIO::ROLE_SCENE_LINEAR, ocio_display_,
                                                    ocio_view_, ocio_look_);
