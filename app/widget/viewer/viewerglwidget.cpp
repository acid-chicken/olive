...
ViewerGLWidget::ViewerGLWidget(QWidget *parent) : QOpenGLWidget(parent), color_manager_(nullptr), has_image_(false) {
...
void ViewerGLWidget::SetupColorProcessor() {
  ...
  if (color_manager_) {
    ...
    try {
      ColorProcessorPtr new_service = ColorProcessor::Create(color_manager_->GetConfig(), OCIO::ROLE_SCENE_LINEAR,
                                                             ocio_display_, ocio_view_, ocio_look_);
