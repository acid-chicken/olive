...
ViewerWidget::ViewerWidget(QWidget *parent) : QWidget(parent), viewer_node_(nullptr), playback_speed_(0) {
  // Set up main layout
...
void ViewerWidget::SetColorMenuEnabled(bool enabled) { gl_widget_->SetColorMenuEnabled(enabled); }
...
void ViewerWidget::UpdateRendererParameters() {
  ...
  video_renderer_->SetParameters(VideoRenderingParams(
      viewer_node_->video_params(), PixelService::instance()->GetConfiguredFormatForMode(render_mode), render_mode, 2));
