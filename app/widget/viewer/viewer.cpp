...
void ViewerWidget::ConnectNodeInternal(ViewerOutput* n) {
  ...
  foreach (ViewerDisplayWidget* glw, gl_widgets_) { glw->ConnectColorManager(using_manager); }
...
void ViewerWidget::DisconnectNodeInternal(ViewerOutput* n) {
  ...
  foreach (ViewerDisplayWidget* glw, gl_widgets_) { glw->DisconnectColorManager(); }
...
const QList<ViewerDisplayWidget*>& ViewerWidget::gl_widgets() const { return gl_widgets_; }

ViewerDisplayWidget* ViewerWidget::main_gl_widget() const { return gl_widgets_.first(); }
...
void ViewerWidget::SetMatrix(const QMatrix4x4& mat) {
  foreach (ViewerDisplayWidget* glw, gl_widgets_) { glw->SetMatrix(mat); }
}
...
ColorManager* ViewerWidget::color_manager() const { return main_gl_widget()->color_manager(); }
...
void ViewerWidget::PlayInternal(int speed, bool in_to_out_only) {
  ...
  if (!audio_fn.isEmpty()) {
    ...
    AudioManager::instance()->StartOutput(audio_fn, audio_renderer_->params().time_to_bytes(GetTime()),
                                          playback_speed_);
  }
...
void ViewerWidget::ContextMenuSetCustomSafeMargins() {
  ...
  forever {
    ...
    s = QInputDialog::getText(this, tr("Safe Margins"), tr("Enter custom ratio (e.g. \"4:3\", \"16/9\", etc.):"),
    ...
    QMessageBox::warning(this, tr("Invalid custom ratio"),
                         tr("Failed to parse \"%1\" into an aspect ratio. Please format a rational fraction with a ':' "
                            "or a '/' separator.")
                             .arg(s),
                         QMessageBox::Ok);
  }
...
void ViewerWidget::UpdateRendererParameters() {
  ...
  AudioRenderingParams aparam(GetConnectedNode()->audio_params(), SampleFormat::kInternalFormat);
...
void ViewerWidget::Play(bool in_to_out_only) {
  if (in_to_out_only && GetConnectedTimelinePoints() && GetConnectedTimelinePoints()->workarea()->enabled()) {
    // Jump to in point
...
void ViewerWidget::SetSignalCursorColorEnabled(bool e) {
  foreach (ViewerDisplayWidget* glw, gl_widgets_) { glw->SetSignalCursorColorEnabled(e); }
}
...
void ViewerWidget::PlaybackTimerUpdate() {
  ...
  {
    if ((play_in_to_out_only_ || Config::Current()["Loop"].toBool()) && GetConnectedTimelinePoints() &&
  ...
  if ((playback_speed_ < 0 && current_time <= min_time) || (playback_speed_ > 0 && current_time >= max_time)) {
