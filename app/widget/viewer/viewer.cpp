...
const double &ViewerWidget::scale() const { return ruler_->scale(); }

rational ViewerWidget::GetTime() const { return Timecode::timestamp_to_time(ruler_->GetTime(), time_base_); }

void ViewerWidget::SetScale(const double &scale_) { ruler_->SetScale(scale_); }
...
bool ViewerWidget::IsPlaying() const { return playback_timer_.isActive(); }
...
ViewerOutput *ViewerWidget::GetConnectedViewer() const { return viewer_node_; }
...
void ViewerWidget::SetMatrix(const QMatrix4x4 &mat) { gl_widget_->SetMatrix(mat); }

VideoRenderBackend *ViewerWidget::video_renderer() const { return video_renderer_; }

void ViewerWidget::SetTexture(OpenGLTexturePtr tex) { gl_widget_->SetTexture(tex); }
...
void ViewerWidget::UpdateRendererParameters() {
  ...
  video_renderer_->SetParameters(VideoRenderingParams(viewer_node_->video_params(),
                                                      ...
                                                      render_mode, divider_));
  audio_renderer_->SetParameters(
      AudioRenderingParams(viewer_node_->audio_params(), SampleFormat::GetConfiguredFormatForMode(render_mode)));
...
void ViewerWidget::SetOCIODisplay(const QString &display) { gl_widget_->SetOCIODisplay(display); }

void ViewerWidget::SetOCIOView(const QString &view) { gl_widget_->SetOCIOView(view); }

void ViewerWidget::SetOCIOLook(const QString &look) { gl_widget_->SetOCIOLook(look); }
...
void ViewerWidget::SizeChangedSlot(int width, int height) { sizer_->SetChildSize(width, height); }
...
void ViewerWidget::ColorDisplayChanged(QAction *action) { SetOCIODisplay(action->data().toString()); }

void ViewerWidget::ColorViewChanged(QAction *action) { SetOCIOView(action->data().toString()); }

void ViewerWidget::ColorLookChanged(QAction *action) { SetOCIOLook(action->data().toString()); }
...
void ViewerWidget::InvalidateVisible() { video_renderer_->InvalidateCache(GetTime(), GetTime()); }
