...
NodeView::NodeView(QWidget* parent) : HandMovableView(parent), graph_(nullptr), drop_edge_(nullptr) {
  setScene(&scene_);
...
void NodeView::DeleteSelected() {
...

  {
    ...
    foreach (NodeEdge* edge, selected_edges) { new NodeEdgeRemoveCommand(edge->output(), edge->input(), command); }
  }
...
void NodeView::SelectAll() { scene_.SelectAll(); }

void NodeView::DeselectAll() { scene_.DeselectAll(); }
      ...
      foreach (NodeEdgePtr edge, src->output()->edges()) {
        if (edge->input()->parentNode() == dst) {
          new NodeEdgeAddCommand(duplicated_nodes.at(i)->output(),
                                 duplicated_nodes.at(j)->GetInputWithID(edge->input()->id()), command);
        }
      }
    }
  }
...
void NodeView::AutoPositionDescendents() {
  ...
  foreach (Node* n, selected) { scene_.ReorganizeFrom(n); }
}
...
void NodeView::ContextMenuLabelNode() {
  ...
  QString s =
      QInputDialog::getText(this, tr("Label Node"), tr("Set node label"), QLineEdit::Normal, n->GetLabel(), &ok);
...
void NodeView::AttachNodesToCursor(const QList<Node*>& nodes) {
  ...
  foreach (Node* p, nodes) { items.append(scene_.NodeToUIObject(p)); }
...
void NodeView::AttachItemsToCursor(const QList<NodeViewItem*>& items) {
  ...
  if (!items.isEmpty()) {
    foreach (NodeViewItem* i, items) { attached_items_.append({i, i->pos() - items.first()->pos()}); }
...
void NodeView::MoveAttachedNodesToCursor(const QPoint& p) {
  ...
  foreach (const AttachedItem& i, attached_items_) { i.item->setPos(item_pos + i.original_pos); }
}
