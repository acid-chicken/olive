...
NodeViewScene::NodeViewScene(QObject *parent)
    : QGraphicsScene(parent), graph_(nullptr), direction_(NodeViewCommon::kLeftToRight) {}
...
NodeViewCommon::FlowDirection NodeViewScene::GetFlowDirection() const { return direction_; }
...
void NodeViewScene::ReorganizeFrom(Node *n) {
  ...
  for (int i = 0; i < immediates.size(); i++) {
    immediates.at(i)->SetPosition(QPointF(child_x, children_y + i));
...
void NodeViewScene::NodeLabelChanged() { item_map_.value(static_cast<Node *>(sender()))->update(); }
