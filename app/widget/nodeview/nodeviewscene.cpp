...
NodeViewScene::NodeViewScene(QObject* parent) : QGraphicsScene(parent), graph_(nullptr) {
...
NodeViewItem* NodeViewScene::NodeToUIObject(Node* n) { return item_map_.value(n); }

NodeViewEdge* NodeViewScene::EdgeToUIObject(NodeEdgePtr n) { return edge_map_.value(n.get()); }

void NodeViewScene::SetGraph(NodeGraph* graph) { graph_ = graph; }
...
const QHash<Node*, NodeViewItem*>& NodeViewScene::item_map() const { return item_map_; }

const QHash<NodeEdge*, NodeViewEdge*>& NodeViewScene::edge_map() const { return edge_map_; }
...
void NodeViewScene::AddNode(Node* node) {
  ...
  foreach (NodeParam* param, node->parameters()) {
    ...
    if (param->type() == NodeParam::kOutput) {
      ...
      foreach (NodeEdgePtr edge, edges) { AddEdge(edge); }
    }
  }
...
void NodeViewScene::RemoveNode(Node* node) { delete item_map_.take(node); }
...
void NodeViewScene::RemoveEdge(NodeEdgePtr edge) { delete edge_map_.take(edge.get()); }
...
QList<Node*> NodeViewScene::GetNodeDirectDescendants(Node* n, const QList<Node*> connected_nodes,
...
int NodeViewScene::FindWeightsInternal(Node* node, QHash<Node*, int>& weights, QList<Node*>& weighted_nodes) {
  ...
  if (!connected_nodes.isEmpty()) {
    ...
    foreach (Node* dep, direct_descendants) { weight += FindWeightsInternal(dep, weights, weighted_nodes); }
  }
...
void NodeViewScene::ReorganizeInternal(NodeViewItem* src_item, QHash<Node*, int>& weights,
  ...
  foreach (Node* dep, direct_descendants) { descendant_weight += weights.value(dep); }

  qreal center_y = src_item->y();
  qreal total_height =
      descendant_weight * src_item->rect().height() + (direct_descendants.size() - 1) * src_item->rect().height() / 2;
  ...
  for (int i = 0; i < direct_descendants.size(); i++) {
    ...
    QPointF item_pos(src_item->pos().x() - item->rect().width() * 3 / 2, item_y);
...
void NodeViewScene::Reorganize() {
  ...
  foreach (Node* end_node, end_nodes) { FindWeightsInternal(end_node, node_weights, processed_nodes); }

  processed_nodes.clear();

  foreach (Node* end_node, end_nodes) { ReorganizeInternal(NodeToUIObject(end_node), node_weights, processed_nodes); }
}
