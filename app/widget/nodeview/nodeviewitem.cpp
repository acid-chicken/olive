...
Node* NodeViewItem::node() { return node_; }

bool NodeViewItem::IsExpanded() { return expanded_; }
...
QRectF NodeViewItem::GetParameterConnectorRect(int index) {
  ...
  QRectF connector_rect(rect().x(),
                        ...
                        node_connector_size_, node_connector_size_);
...
QPointF NodeViewItem::GetParameterTextPoint(int index) {
  ...
  if (param->type() == NodeParam::kOutput) {
    return content_rect_.topRight() +
           QPointF(-(node_connector_size_ + node_text_padding_),
                   node_text_padding_ + font_metrics.ascent() + font_metrics.height() * index);
  } else {
    return content_rect_.topLeft() +
           QPointF(node_connector_size_ + node_text_padding_,
                   node_text_padding_ + font_metrics.ascent() + font_metrics.height() * index);
  }
}
...
void NodeViewItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*) {
  ...
  if (node_ != nullptr) {
    ...
    expand_hitbox_ =
        title_bar_rect_.adjusted(node_icon_padding_, node_icon_padding_, -node_icon_padding_, -node_icon_padding_);
    ...
    QRectF text_rect = title_bar_rect_.adjusted(node_icon_padding_ + expand_hitbox_.width() + node_text_padding_,
                                                node_text_padding_, -node_text_padding_, -node_text_padding_);
  ...
  if (dragging_edge_ != nullptr) {
    ...
    if (drop_item != nullptr && drop_item != drag_source_) {
      ...
      for (int i = 0; i < drop_item->node()->parameters().size(); i++) {
        ...
        QRectF param_hitbox = drop_item->GetParameterConnectorRect(i).adjusted(
            -node_connector_size_, -node_connector_size_, node_connector_size_, node_connector_size_);

        // Get the parameter we're dragging into
        NodeParam* comp_param = drop_item->node()->parameters().at(i);

        if (comp_param->IsConnectable() && param_hitbox.contains(drop_item->mapFromScene(
                                               event->scenePos()))) {  // See if we're dragging inside the hitbox
...
void NodeViewItem::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event) { ToggleExpanded(); }
