...
ManagedDisplayWidget::ManagedDisplayWidget(QWidget* parent)
    : QOpenGLWidget(parent), color_manager_(nullptr), color_service_(nullptr) {
  setContextMenuPolicy(Qt::CustomContextMenu);
}

ManagedDisplayWidget::~ManagedDisplayWidget() { ContextCleanup(); }
...
OpenGLColorProcessorPtr ManagedDisplayWidget::color_service() { return color_service_; }
...
void ManagedDisplayWidget::initializeGL() {
  ...
  connect(context(), &QOpenGLContext::aboutToBeDestroyed, this, &ManagedDisplayWidget::ContextCleanup,
          Qt::DirectConnection);
}
...
void ManagedDisplayWidget::SetupColorProcessor() {
  ...
  if (color_manager_) {
    ...
    try {
      color_service_ =
          OpenGLColorProcessor::Create(color_manager_, color_manager_->GetReferenceColorSpace(), color_transform_);
