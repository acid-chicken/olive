...
KeyframeViewItem::KeyframeViewItem(NodeKeyframePtr key, QGraphicsItem *parent)
    : QGraphicsRectItem(parent), key_(key), scale_(1.0), vert_center_(0), use_custom_brush_(false) {
...
NodeKeyframePtr KeyframeViewItem::key() const { return key_; }
...
void KeyframeViewItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
  ...
  switch (key_->type()) {
    case NodeKeyframe::kLinear: {
      QPointF points[] = {QPointF(rect().center().x(), rect().top()), QPointF(rect().right(), rect().center().y()),
...
void KeyframeViewItem::TimeTargetChangedEvent(Node *) { UpdatePos(); }
...
void KeyframeViewItem::Redraw() { QGraphicsItem::update(); }
