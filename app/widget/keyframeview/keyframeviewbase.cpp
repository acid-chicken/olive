...
const double& KeyframeViewBase::GetYScale() const { return y_scale_; }
...
void KeyframeViewBase::DeleteSelected() {
  ...
  for (i = item_map_.constBegin(); i != item_map_.constEnd(); i++) {
    if (i.value()->isSelected()) {
      ...
      new NodeParamRemoveKeyframeCommand(input_parent, input_parent->get_keyframe_shared_ptr_from_raw(i.key()),
                                         command);
    }
  }
    ...
    if (active_tool_ == Tool::kPointer) {
      ...
      if (item_under_cursor) {
        ...
        if (dragging_bezier_point_) {
          ...
          dragging_bezier_point_opposing_start_ = dragging_bezier_point_->key()->bezier_control(
              NodeKeyframe::get_opposing_bezier_type(dragging_bezier_point_->mode()));
          ...
          for (int i = 0; i < selected_items.size(); i++) {
            ...
            selected_keys_.replace(i, {key, key->x(),
                                       GetAdjustedTime(key->key()->parent()->parentNode(), GetTimeTarget(),
                                                       key->key()->time(), NodeParam::kOutput),
...
void KeyframeViewBase::mouseMoveEvent(QMouseEvent* event) {
  ...
  if (event->buttons() & Qt::LeftButton) {
    ...
    if (active_tool_ == Tool::kPointer) {
      ...
      if (dragging_bezier_point_) {
        ProcessBezierDrag(mouse_diff_scaled, !(event->modifiers() & Qt::ControlModifier), false);
        ...
        foreach (const KeyframeItemAndTime& keypair, selected_keys_) {
          ...
          rational node_time =
              GetAdjustedTime(GetTimeTarget(), keypair.key->key()->parent()->parentNode(),
                              CalculateNewTimeFromScreen(keypair.time, mouse_diff_scaled.x()), NodeParam::kInput);
  ...
  if (event->button() == Qt::LeftButton) {
    ...
    if (active_tool_ == Tool::kPointer) {
      ...
      if (!mouse_diff.isNull()) {
        if (dragging_bezier_point_) {
          ProcessBezierDrag(mouse_diff_scaled, !(event->modifiers() & Qt::ControlModifier), true);
          ...
          foreach (const KeyframeItemAndTime& keypair, selected_keys_) {
            ...
            new NodeParamSetKeyframeTimeCommand(item->key(), node_time, keypair.time, command);

            // Commit value if we're setting a value
            if (y_axis_enabled_) {
              item->key()->set_value(keypair.value);
              new NodeParamSetKeyframeValueCommand(item->key(), keypair.value - mouse_diff_scaled.y(), keypair.value,
                                                   command);
            }
...
const QMap<NodeKeyframe*, KeyframeViewItem*>& KeyframeViewBase::item_map() const { return item_map_; }
...
void KeyframeViewBase::SetYAxisEnabled(bool e) { y_axis_enabled_ = e; }
...
QPointF KeyframeViewBase::GenerateBezierControlPosition(const NodeKeyframe::BezierType mode, const QPointF& start_point,
...
void KeyframeViewBase::ProcessBezierDrag(QPointF mouse_diff_scaled, bool include_opposing, bool undoable) {
  ...
  QPointF new_bezier_pos =
      GenerateBezierControlPosition(dragging_bezier_point_->mode(), dragging_bezier_point_start_, mouse_diff_scaled);
  ...
  if (undoable) {
    ...
    dragging_bezier_point_->key()->set_bezier_control(dragging_bezier_point_->mode(), dragging_bezier_point_start_);

    new KeyframeSetBezierControlPoint(dragging_bezier_point_->key(), dragging_bezier_point_->mode(), new_bezier_pos,
                                      dragging_bezier_point_start_, command);

    if (include_opposing) {
      dragging_bezier_point_->key()->set_bezier_control(opposing_type, dragging_bezier_point_opposing_start_);

      new KeyframeSetBezierControlPoint(dragging_bezier_point_->key(), opposing_type, new_opposing_pos,
                                        dragging_bezier_point_opposing_start_, command);
    }
    ...
    dragging_bezier_point_->key()->set_bezier_control(dragging_bezier_point_->mode(), new_bezier_pos);

    dragging_bezier_point_->key()->set_bezier_control(opposing_type, new_opposing_pos);
...
QPointF KeyframeViewBase::GetScaledCursorPos(const QPoint& cursor_pos) {
  return QPointF(static_cast<double>(cursor_pos.x()) / GetScale(), static_cast<double>(cursor_pos.y()) / y_scale_);
}
  ...
  if (!items.isEmpty()) {
    if (selected == linear_key_action || selected == bezier_key_action || selected == hold_key_action) {
      ...
      foreach (QGraphicsItem* item, items) {
        new KeyframeSetTypeCommand(static_cast<KeyframeViewItem*>(item)->key(), new_type, command);
      }
      Core::instance()->undo_stack()->pushIfHasChildren(command);
    }
  }
...
void KeyframeViewBase::ShowKeyframePropertiesDialog() {
  ...
  foreach (QGraphicsItem* item, items) { keys.append(static_cast<KeyframeViewItem*>(item)->key()); }
