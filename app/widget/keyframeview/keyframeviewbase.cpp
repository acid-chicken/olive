...
KeyframeViewBase::KeyframeViewBase(QWidget* parent)
    : TimelineViewBase(parent), y_scale_(1.0), dragging_bezier_point_(nullptr), y_axis_enabled_(false) {
      ...
      if (!mouse_diff.isNull()) {
        if (dragging_bezier_point_) {
          ...
          foreach (const KeyframeItemAndTime& keypair, selected_keys_) {
            ...
            new NodeParamSetKeyframeTimeCommand(item->key(), new_time, keypair.time, command);
