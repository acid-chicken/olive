...
bool Core::Start() {
  ...
  //               the fact that some of the config paths set by default rely on the app name having been set (in
  //               main())
  ...
  if (gui_active_) {
    ...
    if (!startup_project_.isEmpty() && !QFileInfo::exists(startup_project_)) {
      QMessageBox::warning(
          main_window(), tr("Failed to open startup file"),
...
const Tool::AddableObject& Core::selected_addable_object() const { return addable_object_; }

void Core::SetSelectedAddableObject(const Tool::AddableObject& obj) { addable_object_ = obj; }

void Core::ClearOpenRecentList() { recent_projects_.clear(); }
...
void Core::CreateNewSequence() {
  ...
  if (sd.exec() == QDialog::Accepted) {
    // Create an undoable command
    ProjectViewModel::AddItemCommand* aic =
        new ProjectViewModel::AddItemCommand(GetActiveProjectModel(), GetSelectedFolderInActiveProject(), new_sequence);
...
bool Core::CloseActiveProject() { return CloseProject(GetActiveProject(), true); }
    ...
    if (open_projects_.at(i) == p) {
      if (p->is_modified() && confirm_behavior != kCloseProjectDontSave) {
        ...
        if (confirm_behavior == kCloseProjectAsk || confirm_behavior == kCloseProjectOnlyOne) {
          ...
          mb.setText(tr("The project '%1' has unsaved changes. Would you like to save them?").arg(p->name()));
...
bool Core::CloseAllProjects() { return CloseAllProjects(true); }
