...
PreferencesAudioTab::PreferencesAudioTab() : has_devices_(false) {
...
void PreferencesAudioTab::RetrieveDeviceLists() {
  ...
  for (int i = 0; i < AudioManager::instance()->ListOutputDevices().size(); i++) {
    audio_output_devices->addItem(AudioManager::instance()->ListOutputDevices().at(i).deviceName(), i);
  ...
  for (int i = 0; i < AudioManager::instance()->ListInputDevices().size(); i++) {
    audio_input_devices->addItem(AudioManager::instance()->ListInputDevices().at(i).deviceName(), i);
    if (!found_preferred_device &&
