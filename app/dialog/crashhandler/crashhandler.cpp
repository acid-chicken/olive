...
CrashHandlerDialog::CrashHandlerDialog(const char* log_file) {
  ...
  layout->addWidget(
      new QLabel(tr("We're sorry, Olive has crashed. Please send the following log to the developers to "
                    "help resolve this.")));
  ...
  edit->append(
      QStringLiteral("Build Environment: %1 (%2)").arg(QSysInfo::buildCpuArchitecture(), QSysInfo::buildAbi()));
  ...
  edit->append(QStringLiteral("System: %1 (%2 %3)")
  ...
  if (log.open(QFile::ReadOnly | QFile::Text)) {
    ...
    QMetaObject::invokeMethod(edit->verticalScrollBar(), "setValue", Qt::QueuedConnection, Q_ARG(int, 0));

    log.close();
  }
}

void CrashHandlerDialog::accept() { QDialog::accept(); }

void CrashHandlerDialog::reject() { QDialog::reject(); }
