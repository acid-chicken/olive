...
SpeedDurationDialog::SpeedDurationDialog(const rational& timebase, const QList<ClipBlock*>& clips, QWidget* parent)
    : QDialog(parent), clips_(clips), timebase_(timebase) {
...

  {
    ...
    for (int i = 1; i < clips.size(); i++) {
      ...
      if (same_speed && prev_clip->speed() == this_clip->speed()) {
        same_speed = false;
      }

      // Check if the durations are different
      if (same_duration && prev_clip->length() != this_clip->length()) {
        same_duration = false;
      }

      // Check if all are reversed
      if (all_reversed && prev_clip->is_reversed() != this_clip->is_reversed()) {
        all_reversed = false;
      }

      // If we've already determined both are different, no need to continue
      if (!same_speed && !same_duration && !all_reversed) {
        break;
      }
    }
  ...
  foreach (ClipBlock* clip, clips_) {
    ...
    if (change_duration) {
      ...
      if (new_duration != current_duration) {
        ...
        if (!ripple_clips_checkbox_->isChecked() && next_block) {
...
BlockReverseCommand::BlockReverseCommand(Block* block, QUndoCommand* parent) : UndoCommand(parent), block_(block) {}

Project* BlockReverseCommand::GetRelevantProject() const { return static_cast<Sequence*>(block_->parent())->project(); }
...
void BlockReverseCommand::undo_internal() { redo_internal(); }
