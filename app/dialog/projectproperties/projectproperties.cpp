...
ProjectPropertiesDialog::ProjectPropertiesDialog(Project* p, QWidget* parent)
    : QDialog(parent), working_project_(p), ocio_config_is_valid_(true) {
  ...
  if (working_project_ == nullptr) {
    QMessageBox::critical(this, tr("No Active Project"), tr("No project is currently open to set the properties for"),
                          QMessageBox::Ok);
    reject();
    return;
  }
...
void ProjectPropertiesDialog::accept() {
  if (ocio_config_is_valid_) {
    ...
    QMessageBox::critical(this, tr("OpenColorIO Config Error"),
                          tr("Failed to set OpenColorIO configuration: %1").arg(ocio_config_error_), QMessageBox::Ok);
  }
}
