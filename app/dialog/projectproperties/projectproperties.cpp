...
PathWidget::PathWidget(const QString& path, QWidget* parent) : QObject(parent) {
...
bool PathWidget::PathIsValid(bool try_to_create) const {
  return default_box_->isChecked() || QDir(path_edit_->text()).exists() ||
         (try_to_create && QDir(path_edit_->text()).mkpath(QStringLiteral(".")));
}

void PathWidget::DefaultToggled(bool e) { path_edit_->setEnabled(!e); }
