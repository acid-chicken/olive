...
H264Section::H264Section(QWidget* parent) : CodecSection(parent) {
...
void H264Section::AddOpts(EncodingParams* params) {
  ...
  if (method == kConstantRateFactor) {
    ...
    if (method == kTargetBitRate) {
      ...
      target_rate =
          qRound64(static_cast<double>(filesize_section_->GetFileSize()) / params->GetExportLength().toDouble());
...
H264CRFSection::H264CRFSection(QWidget* parent) : QWidget(parent) {
