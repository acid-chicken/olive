...
ExportVideoTab::ExportVideoTab(ColorManager *color_manager, QWidget *parent)
    : QWidget(parent), color_manager_(color_manager), threads_(0) {
...
QComboBox *ExportVideoTab::codec_combobox() const { return codec_combobox_; }

IntegerSlider *ExportVideoTab::width_slider() const { return width_slider_; }

IntegerSlider *ExportVideoTab::height_slider() const { return height_slider_; }

QCheckBox *ExportVideoTab::maintain_aspect_checkbox() const { return maintain_aspect_checkbox_; }

QComboBox *ExportVideoTab::scaling_method_combobox() const { return scaling_method_combobox_; }

const rational &ExportVideoTab::frame_rate() const { return frame_rates_.at(frame_rate_combobox_->currentIndex()); }
...
QString ExportVideoTab::CurrentOCIOColorSpace() { return color_space_chooser_->input(); }

CodecSection *ExportVideoTab::GetCodecSection() const {
  return static_cast<CodecSection *>(codec_stack_->currentWidget());
}

void ExportVideoTab::SetCodecSection(CodecSection *section) { codec_stack_->setCurrentWidget(section); }

ImageSection *ExportVideoTab::image_section() const { return image_section_; }

H264Section *ExportVideoTab::h264_section() const { return h264_section_; }

const int &ExportVideoTab::threads() const { return threads_; }
...
QWidget *ExportVideoTab::SetupResolutionSection() {
  ...
  foreach (const rational &fr, frame_rates_) { frame_rate_combobox_->addItem(Core::FrameRateToString(fr)); }
