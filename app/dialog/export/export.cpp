...
void ExportDialog::accept() {
  ...
  if (file_info.exists()) {
    ...
    b.setText(tr("The file \"%1\" already exists. Do you want to overwrite it?").arg(filename_edit_->text()));
    b.addButton(QMessageBox::Yes);
    b.addButton(QMessageBox::No);
  ...
  if (video_enabled_->isChecked()) {
    if (video_tab_->width_slider()->GetValue() % 2 != 0 || video_tab_->height_slider()->GetValue() % 2 != 0) {
      QMessageBox b(this);
      b.setIcon(QMessageBox::Critical);
      b.setWindowModality(Qt::WindowModal);
      b.setWindowTitle(tr("Invalid parameters"));
...
void ExportDialog::BrowseFilename() {
  ...
  QString browsed_fn = QFileDialog::getSaveFileName(
      this, "", filename_edit_->text(),
      QStringLiteral("%1 (*.%2)").arg(current_format.name(), current_format.extension()), nullptr,
...
int ExportDialog::AlignEvenNumber(double d) { return qCeil(d * 0.5) * 2; }

ExportParams ExportDialog::GenerateParams() const {
  RenderMode::Mode render_mode = RenderMode::kOnline;

  VideoRenderingParams video_render_params(
      static_cast<int>(video_tab_->width_slider()->GetValue()),
      static_cast<int>(video_tab_->height_slider()->GetValue()), video_tab_->frame_rate().flipped(),
      PixelFormat::instance()->GetConfiguredFormatForMode(render_mode), render_mode);
  ...
  if (video_enabled_->isChecked()) {
    ...
    params.EnableVideo(video_render_params, video_codec.id());
  ...
  if (audio_enabled_->isChecked()) {
    ...
    params.EnableAudio(audio_render_params, audio_codec.id());
  }
...
QString ExportDialog::TimeToString(int64_t ms) {
  ...
  return QStringLiteral("%1:%2:%3").arg(hh, 2, 10, QChar('0')).arg(mm, 2, 10, QChar('0')).arg(ss, 2, 10, QChar('0'));
}
...
void ExportDialog::UpdateViewerDimensions() {
  ...
  preview_viewer_->SetMatrix(Exporter::GenerateMatrix(
      static_cast<ExportParams::VideoScalingMethod>(video_tab_->scaling_method_combobox()->currentData().toInt()),
