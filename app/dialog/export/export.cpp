...
ExportDialog::ExportDialog(ViewerOutput* viewer_node, QWidget* parent)
    : QDialog(parent), viewer_node_(viewer_node), previously_selected_format_(0) {
  ...
  foreach (const ExportFormat& format, formats_) { format_combobox_->addItem(format.name()); }
  ...
  video_aspect_ratio_ = static_cast<double>(viewer_node_->video_params().width()) /
                        static_cast<double>(viewer_node_->video_params().height());
  ...
  connect(video_tab_->scaling_method_combobox(), SIGNAL(currentIndexChanged(int)), this,
          SLOT(UpdateViewerDimensions()));
  ...
  preview_viewer_->SetOCIOParameters(video_tab_->CurrentOCIODisplay(), video_tab_->CurrentOCIOView(),
...
void ExportDialog::accept() {
  if (!video_enabled_->isChecked() && !audio_enabled_->isChecked()) {
    QMessageBox::warning(this, tr("Invalid parameters"),
                         tr("Both video and audio are disabled. There's nothing to export."), QMessageBox::Ok);
    return;
  }
  ...
  if (video_tab_->scaling_method_combobox()->isEnabled()) {
    ...
    transform = GenerateMatrix(
        static_cast<ExportVideoTab::ScalingMethod>(video_tab_->scaling_method_combobox()->currentData().toInt()),
        source_width, source_height, dest_width, dest_height);
  }

  RenderMode::Mode render_mode = RenderMode::kOnline;

  VideoRenderingParams video_render_params(dest_width, dest_height, video_tab_->frame_rate().flipped(),
  ...
  ColorProcessorPtr color_processor =
      ColorProcessor::Create(color_manager_->GetConfig(), OCIO::ROLE_SCENE_LINEAR, video_tab_->CurrentOCIODisplay(),
                             video_tab_->CurrentOCIOView(), video_tab_->CurrentOCIOLook());
  ...
  if (video_enabled_->isChecked()) {
    ...
    encoding_params.EnableVideo(video_render_params, video_codec.id());
  }

  if (audio_enabled_->isChecked()) {
    const ExportCodec& audio_codec = codecs_.at(audio_tab_->codec_combobox()->currentData().toInt());
    encoding_params.EnableAudio(audio_render_params, audio_codec.id());
  }
...
void ExportDialog::BrowseFilename() {
  ...
  QString browsed_fn =
      QFileDialog::getSaveFileName(this, "", filename_edit_->text(),
...
void ExportDialog::ResolutionChanged() {
  if (video_tab_->maintain_aspect_checkbox()->isChecked()) {
    ...
    if (sender() == video_tab_->height_slider()) {
      video_tab_->width_slider()->SetValue(
          qRound(static_cast<double>(video_tab_->height_slider()->GetValue()) * video_aspect_ratio_));
    } else {
      // This catches both the width slider changing and the maintain aspect ratio checkbox changing
      video_tab_->height_slider()->SetValue(
          qRound(static_cast<double>(video_tab_->width_slider()->GetValue()) / video_aspect_ratio_));
    }
  }
  ...
  for (int i = 0; i < kFormatCount; i++) {
    switch (static_cast<Format>(i)) {
      ...
      case kFormatMatroska:
        formats_.append(ExportFormat(tr("Matroska Video"), "mkv", "ffmpeg", {kCodecH264, kCodecH265},
                                     {kCodecAAC, kCodecMP2, kCodecMP3, kCodecPCM}));
        break;
      case kFormatMPEG4:
        formats_.append(ExportFormat(tr("MPEG-4 Video"), "mp4", "ffmpeg", {kCodecH264, kCodecH265},
                                     {kCodecAAC, kCodecMP2, kCodecMP3, kCodecPCM}));
        break;
      case kFormatOpenEXR:
      ...
      case kFormatQuickTime:
        formats_.append(ExportFormat(tr("QuickTime"), "mov", "ffmpeg", {kCodecH264, kCodecH265, kCodecProRes},
                                     {kCodecAAC, kCodecMP2, kCodecMP3, kCodecPCM}));
        break;
      case kFormatPNG:
...
QMatrix4x4 ExportDialog::GenerateMatrix(ExportVideoTab::ScalingMethod method, int source_width, int source_height,
...
void ExportDialog::UpdateViewerDimensions() {
  ...
  preview_viewer_->SetMatrix(GenerateMatrix(
      static_cast<ExportVideoTab::ScalingMethod>(video_tab_->scaling_method_combobox()->currentData().toInt()),
      viewer_node_->video_params().width(), viewer_node_->video_params().height(),
      video_tab_->width_slider()->GetValue(), video_tab_->height_slider()->GetValue()));
}
...
void ExportDialog::ExporterIsDone() {
  ...
  if (exporter->GetExportStatus()) {
    QMessageBox::information(this, tr("Export Status"), tr("Export completed successfully!"), QMessageBox::Ok);
  } else {
    QMessageBox::critical(this, tr("Export Status"), tr("Export failed: %1").arg(exporter->GetExportError()),
                          QMessageBox::Ok);
  }
}
