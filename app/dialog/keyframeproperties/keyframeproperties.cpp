...
KeyframePropertiesDialog::KeyframePropertiesDialog(const QList<NodeKeyframePtr>& keys, const rational& timebase,
                                                   QWidget* parent)
...
void KeyframePropertiesDialog::accept() {
  ...
  foreach (NodeKeyframePtr key, keys_) {
    ...
    if (bezier_group_->isEnabled()) {
      new KeyframeSetBezierControlPoint(key, NodeKeyframe::kInHandle,
                                        QPointF(bezier_in_x_slider_->GetValue(), bezier_in_y_slider_->GetValue()),
                                        command);

      new KeyframeSetBezierControlPoint(key, NodeKeyframe::kOutHandle,
