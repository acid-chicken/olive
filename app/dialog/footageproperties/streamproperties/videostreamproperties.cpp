...
VideoStreamProperties::VideoStreamProperties(ImageStreamPtr stream) : stream_(stream) {
  ...
  video_color_space_->addItem(
      tr("Default (%1)").arg(stream->footage()->project()->color_manager()->GetDefaultInputColorSpace()));
...
void VideoStreamProperties::Accept(QUndoCommand *parent) {
  ...
  if (video_premultiply_alpha_->isChecked() != stream_->premultiplied_alpha() ||
    ...
    new VideoStreamChangeCommand(stream_, video_premultiply_alpha_->isChecked(), set_colorspace, parent);
  }
  ...
  if (IsImageSequence(stream_.get())) {
    ...
    if (video_stream->start_time() != imgseq_start_time_->GetValue() || video_stream->duration() != new_dur) {
      new ImageSequenceChangeCommand(video_stream, imgseq_start_time_->GetValue(), new_dur, parent);
    }
  }
}
...
VideoStreamProperties::VideoStreamChangeCommand::VideoStreamChangeCommand(ImageStreamPtr stream, bool premultiplied,
                                                                          QString colorspace, QUndoCommand *parent)
    : UndoCommand(parent), stream_(stream), new_premultiplied_(premultiplied), new_colorspace_(colorspace) {}
...
VideoStreamProperties::ImageSequenceChangeCommand::ImageSequenceChangeCommand(VideoStreamPtr video_stream,
                                                                              int64_t start_index, int64_t duration,
                                                                              QUndoCommand *parent)
    : UndoCommand(parent), video_stream_(video_stream), new_start_index_(start_index), new_duration_(duration) {}
