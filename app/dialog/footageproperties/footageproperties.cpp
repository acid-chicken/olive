...
void FootagePropertiesDialog::accept() {
  ...
  if (footage_->name() != footage_name_field_->text()) {
    new FootageChangeCommand(footage_, footage_name_field_->text(), command);
  }
  ...
  for (int i = 0; i < footage_->streams().size(); i++) {
    ...
    if (footage_->stream(i)->enabled() != stream_enabled) {
      new StreamEnableChangeCommand(footage_->stream(i), stream_enabled, command);
    }
  }
...
FootagePropertiesDialog::FootageChangeCommand::FootageChangeCommand(Footage *footage, const QString &name,
    ...
    : UndoCommand(command), footage_(footage), new_name_(name) {}

Project *FootagePropertiesDialog::FootageChangeCommand::GetRelevantProject() const { return footage_->project(); }
...
void FootagePropertiesDialog::FootageChangeCommand::undo_internal() { footage_->set_name(old_name_); }

FootagePropertiesDialog::StreamEnableChangeCommand::StreamEnableChangeCommand(StreamPtr stream, bool enabled,
                                                                              QUndoCommand *command)
    : UndoCommand(command), stream_(stream), old_enabled_(stream->enabled()), new_enabled_(enabled) {}

Project *FootagePropertiesDialog::StreamEnableChangeCommand::GetRelevantProject() const {
  return stream_->footage()->project();
}

void FootagePropertiesDialog::StreamEnableChangeCommand::redo_internal() { stream_->set_enabled(new_enabled_); }

void FootagePropertiesDialog::StreamEnableChangeCommand::undo_internal() { stream_->set_enabled(old_enabled_); }
