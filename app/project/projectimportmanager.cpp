...
ProjectImportManager::ProjectImportManager(ProjectViewModel *model, Folder *folder, const QStringList &filenames)
    : model_(model), folder_(folder) {
  foreach (const QString &f, filenames) { filenames_.append(f); }
...
const int &ProjectImportManager::GetFileCount() { return file_count_; }
...
void ProjectImportManager::Import(Folder *folder, const QFileInfoList &import, int &counter,
  ...
  foreach (const QFileInfo &file_info, import) {
    ...
    if (file_info.isDir()) {
      ...
      if (!entry_list.isEmpty()) {
        ...
        new ProjectViewModel::AddItemCommand(model_, folder, f, parent_command);
