...
void Node::Load(QXmlStreamReader *reader, QHash<quintptr, NodeOutput *> &output_ptrs,
                QList<NodeInput::SerializedConnection> &input_connections,
                QList<NodeParam::FootageConnection> &footage_connections, const QString &element) {
...
NodeValueTable Node::Value(const NodeValueDatabase &value) const { return value.Merge(); }
...
void Node::InvalidateVisible(NodeInput *from) {
  ...
  foreach (NodeParam *param, params_) {
    if (param->type() == NodeParam::kOutput) {
      foreach (NodeEdgePtr edge, param->edges()) { edge->input()->parentNode()->InvalidateVisible(edge->input()); }
    }
  }
}
...
void Node::LockUserInput() { user_input_lock_.lock(); }

void Node::UnlockUserInput() { user_input_lock_.unlock(); }
...
void DuplicateConnectionsBetweenListsInternal(const QList<Node *> &source, const QList<Node *> &destination,
                                              NodeInput *source_input, NodeInput *dest_input) {
...
void Node::TraverseInputInternal(QList<Node *> &list, NodeInput *input, bool traverse, bool exclusive_only) {
  if (input->IsConnected() && (input->get_connected_output()->edges().size() == 1 || !exclusive_only)) {
...
bool Node::IsAccelerated() const { return false; }

QString Node::AcceleratedCodeVertex() const { return QString(); }

QString Node::AcceleratedCodeFragment() const { return QString(); }

int Node::AcceleratedCodeIterations() const { return 1; }

NodeInput *Node::AcceleratedCodeIterativeInput() const { return nullptr; }

NodeInput *Node::ProcessesSamplesFrom() const { return nullptr; }

void Node::ProcessSamples(const NodeValueDatabase *values, const AudioRenderingParams &params, const float *input,
...
const QPointF &Node::GetPosition() { return position_; }

void Node::SetPosition(const QPointF &pos) { position_ = pos; }
