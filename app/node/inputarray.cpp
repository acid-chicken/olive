...
NodeInputArray::NodeInputArray(const QString &id, const DataType &type, const QVariant &default_value)
    : NodeInput(id, type, default_value), default_value_(default_value) {}

bool NodeInputArray::IsArray() { return true; }

int NodeInputArray::GetSize() const { return sub_params_.size(); }

void NodeInputArray::Prepend() { InsertAt(0); }
...
bool NodeInputArray::ContainsSubParameter(NodeInput *input) const { return sub_params_.contains(input); }

int NodeInputArray::IndexOfSubParameter(NodeInput *input) const { return sub_params_.indexOf(input); }

NodeInput *NodeInputArray::At(int index) const { return sub_params_.at(index); }

NodeInput *NodeInputArray::First() const { return sub_params_.first(); }

NodeInput *NodeInputArray::Last() const { return sub_params_.last(); }

const QVector<NodeInput *> &NodeInputArray::sub_params() { return sub_params_; }
...
void NodeInputArray::InsertAt(int index) {
  ...
  for (int i = sub_params_.size() - 1; i > index; i--) {
    ...
    if (this_param->IsConnected()) {
      // Disconnect whatever is at this parameter (presumably its connection has already been copied so we can just
      // remove it)
    ...
    if (prev_param->IsConnected()) {
      ...
      NodeParam::ConnectEdge(edge->output(), this_param);
    }
  }
}

void NodeInputArray::Append() { SetSize(GetSize() + 1); }

void NodeInputArray::RemoveLast() { SetSize(GetSize() - 1); }
...
void NodeInputArray::LoadInternal(QXmlStreamReader *reader, QHash<quintptr, NodeOutput *> &param_ptrs,
                                  QList<SerializedConnection> &input_connections,
                                  QList<FootageConnection> &footage_connections, const QAtomicInt *cancelled) {
...
void NodeInputArray::SaveInternal(QXmlStreamWriter *writer) const {
  ...
  foreach (NodeInput *sub, sub_params_) { sub->Save(writer); }
