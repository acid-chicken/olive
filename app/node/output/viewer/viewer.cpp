...
Node *ViewerOutput::copy() const { return new ViewerOutput(); }

QString ViewerOutput::Name() const { return tr("Viewer"); }

QString ViewerOutput::id() const { return "org.olivevideoeditor.Olive.vieweroutput"; }

QString ViewerOutput::Category() const { return tr("Output"); }

QString ViewerOutput::Description() const { return tr("Interface between a Viewer panel and the node system."); }

NodeInput *ViewerOutput::texture_input() { return texture_input_; }

NodeInput *ViewerOutput::samples_input() { return samples_input_; }

NodeInput *ViewerOutput::length_input() { return length_input_; }
...
const VideoParams &ViewerOutput::video_params() const { return video_params_; }

const AudioParams &ViewerOutput::audio_params() const { return audio_params_; }
...
void ViewerOutput::set_audio_params(const AudioParams &audio) { audio_params_ = audio; }
...
const QUuid &ViewerOutput::uuid() const { return uuid_; }
...
void ViewerOutput::UpdateLength(const rational &length) {
  ...
  foreach (TrackList *list, track_lists_) { new_length = qMax(new_length, list->TrackLength()); }
...
void ViewerOutput::Retranslate() {
  for (int i = 0; i < track_inputs_.size(); i++) {
    ...
    if (!input_name.isEmpty()) track_inputs_.at(i)->set_name(input_name);
  }
}

const QString &ViewerOutput::media_name() const { return media_name_; }
...
const QVector<TrackOutput *> &ViewerOutput::Tracks() const { return track_cache_; }

NodeInput *ViewerOutput::track_input(Timeline::TrackType type) const { return track_inputs_.at(type); }

TrackList *ViewerOutput::track_list(Timeline::TrackType type) const { return track_lists_.at(type); }
