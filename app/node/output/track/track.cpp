...
TrackOutput::TrackOutput()
    : track_type_(Timeline::kTrackTypeNone), block_invalidate_cache_stack_(0), index_(-1), locked_(false) {
...
void TrackOutput::set_track_type(const Timeline::TrackType &track_type) { track_type_ = track_type; }

const Timeline::TrackType &TrackOutput::track_type() { return track_type_; }

Block::Type TrackOutput::type() const { return kClip; }

Block *TrackOutput::copy() const { return new TrackOutput(); }

QString TrackOutput::Name() const { return tr("Track"); }

QString TrackOutput::id() const { return "org.olivevideoeditor.Olive.track"; }

QString TrackOutput::Category() const { return tr("Output"); }
...
const int &TrackOutput::GetTrackHeight() const { return track_height_; }
...
const int &TrackOutput::Index() { return index_; }

void TrackOutput::SetIndex(const int &index) { index_ = index; }
...
Block *TrackOutput::BlockAtTime(const rational &time) const {
  ...
  foreach (Block *block, block_cache_) {
    if (block && block->in() <= time && block->out() > time) {
      return block;
    }
  }
...
QList<Block *> TrackOutput::BlocksAtTimeRange(const TimeRange &range) const {
  ...
  foreach (Block *block, block_cache_) {
    if (block && block->out() > range.in() && block->in() < range.out()) {
      list.append(block);
    }
  }
...
const QVector<Block *> &TrackOutput::Blocks() const { return block_cache_; }
...
void TrackOutput::PrependBlock(Block *block) { InsertBlockAtIndex(block, 0); }

void TrackOutput::InsertBlockAtIndex(Block *block, int index) {
  BlockInvalidateCache();

  block_input_->InsertAt(index);
  NodeParam::ConnectEdge(block->output(), block_input_->At(index));
...
void TrackOutput::AppendBlock(Block *block) {
  ...
  block_input_->Append();
  NodeParam::ConnectEdge(block->output(), block_input_->At(last_index));
...
void TrackOutput::BlockInvalidateCache() { block_invalidate_cache_stack_++; }

void TrackOutput::UnblockInvalidateCache() { block_invalidate_cache_stack_--; }
...
void TrackOutput::ReplaceBlock(Block *old, Block *replace) {
  ...
  NodeParam::DisconnectEdge(old->output(), block_input_->At(index_of_old_block));

  NodeParam::ConnectEdge(replace->output(), block_input_->At(index_of_old_block));
...
const rational &TrackOutput::track_length() const { return track_length_; }

bool TrackOutput::IsTrack() const { return true; }

int TrackOutput::GetTrackHeightIncrement() { return qApp->fontMetrics().height() / 2; }

int TrackOutput::GetDefaultTrackHeight() { return qApp->fontMetrics().height() * 3; }

int TrackOutput::GetTrackHeightMinimum() { return qApp->fontMetrics().height() * 3 / 2; }
...
QString TrackOutput::GetDefaultTrackName(Timeline::TrackType type, int index) {
  ...
  switch (type) {
    case Timeline::kTrackTypeVideo:
      return tr("Video %1").arg(user_friendly_index);
    case Timeline::kTrackTypeAudio:
      return tr("Audio %1").arg(user_friendly_index);
...
bool TrackOutput::IsMuted() const { return muted_input_->get_standard_value().toBool(); }

bool TrackOutput::IsLocked() const { return locked_; }

void TrackOutput::SetTrackName(const QString &name) { track_name_ = name; }
...
void TrackOutput::SetLocked(bool e) { locked_ = e; }
...
void TrackOutput::UpdatePreviousAndNextOfIndex(int index) {
  ...
  if (ref) {
    ...
    if (previous) previous->set_next(ref);

    if (next) next->set_previous(ref);
  } else {
    // Link previous and next together
    if (previous) previous->set_next(next);

    if (next) next->set_previous(previous);
  }
}
