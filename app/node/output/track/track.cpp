...
QString TrackOutput::id() const { return QStringLiteral("org.olivevideoeditor.Olive.track"); }
...
const QList<Block *> &TrackOutput::Blocks() const { return block_cache_; }
...
void TrackOutput::InsertBlockAtIndex(Block *block, int index) {
  ...
  NodeParam::ConnectEdge(block->output(), block_input_->At(insert_index));
...
NodeInputArray *TrackOutput::block_input() const { return block_input_; }
...
void TrackOutput::UpdateInOutFrom(int index) {
  ...
  if (last_out != track_length_) {
    ...
    InvalidateCache(TimeRange(qMin(old_track_length, last_out), qMax(old_track_length, last_out)), block_input_,
                    block_input_);
  }
}
...
void TrackOutput::MutedInputValueChanged() { emit MutedChanged(IsMuted()); }
