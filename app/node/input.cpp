...
NodeInput::NodeInput(const QString &id, const DataType &type, const QVariant &default_value)
    : NodeParam(id), data_type_(type), keyframable_(true), keyframing_(false) {
  int track_size;
...
bool NodeInput::IsArray() { return false; }
...
void NodeInput::Load(QXmlStreamReader *reader, QHash<quintptr, NodeOutput *> &param_ptrs,
                     QList<SerializedConnection> &input_connections, QList<FootageConnection> &footage_connections,
...
void NodeInput::LoadInternal(QXmlStreamReader *, QHash<quintptr, NodeOutput *> &, QList<SerializedConnection> &,
                             QList<FootageConnection> &, const QAtomicInt *) {}
...
void NodeInput::emit_time_range(const TimeRange &range) { emit ValueChanged(range.in(), range.out()); }
