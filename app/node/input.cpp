...
NodeInput::NodeInput(const QString &id, const DataType &type, const QVector<QVariant> &default_value) : NodeParam(id) {
  Init(type);
...
NodeInput::NodeInput(const QString &id, const NodeParam::DataType &type) : NodeParam(id) { Init(type); }

bool NodeInput::IsArray() const { return false; }

NodeParam::Type NodeInput::type() { return kInput; }
...
void NodeInput::Save(QXmlStreamWriter *writer) const {
  ...
  foreach (const QVariant &v, standard_value_) { writer->writeTextElement("value", ValueToString(v)); }
...
void NodeInput::SaveConnections(QXmlStreamWriter *writer) const {
  ...
  foreach (NodeEdgePtr edge, edges_) {
    writer->writeTextElement("connection", QString::number(reinterpret_cast<quintptr>(edge->output())));
  }

  writer->writeEndElement();  // connections
}

const NodeParam::DataType &NodeInput::data_type() const { return data_type_; }
...
QString NodeInput::ValueToString(const QVariant &value) const { return ValueToString(data_type_, value); }
...
void NodeInput::GetDependencies(QList<Node *> &list, bool traverse, bool exclusive_only) const {
  if (IsConnected() && (get_connected_output()->edges().size() == 1 || !exclusive_only)) {
    ...
    if (!list.contains(connected)) {
      ...
      if (traverse) {
        ...
        foreach (NodeInput *i, connected_inputs) { i->GetDependencies(list, traverse, exclusive_only); }
      }
    }
  }
}
...
QList<Node *> NodeInput::GetExclusiveDependencies() const { return GetDependencies(true, true); }

QList<Node *> NodeInput::GetImmediateDependencies() const { return GetDependencies(false, false); }
...
bool NodeInput::type_can_be_interpolated(NodeParam::DataType type) {
  return type == kFloat || type == kVec2 || type == kVec3 || type == kVec4 || type == kColor;
}
...
QVariant NodeInput::get_value_at_time_for_track(const rational &time, int track) const {
  if (!is_using_standard_value(track)) {
    ...
    for (int i = 0; i < key_track.size() - 1; i++) {
      ...
      if (before->time() == time || !type_can_be_interpolated(data_type()) ||
        ...
        if (before->type() == NodeKeyframe::kBezier && after->type() == NodeKeyframe::kBezier) {
          ...
          double t = Bezier::CubicXtoT(
              time.toDouble(), before->time().toDouble(), before->time().toDouble() + before->bezier_control_out().x(),
              after->time().toDouble() + after->bezier_control_in().x(), after->time().toDouble());

          double y = Bezier::CubicTtoY(
              before->value().toDouble(), before->value().toDouble() + before->bezier_control_out().y(),
              after->value().toDouble() + after->bezier_control_in().y(), after->value().toDouble(), t);
          ...
          double t = Bezier::QuadraticXtoT(time.toDouble(), before->time().toDouble(), control_point_time,
                                           after->time().toDouble());

          // Generate value using T
          double y =
              Bezier::QuadraticTtoY(before->value().toDouble(), control_point_value, after->value().toDouble(), t);
...
int NodeInput::get_number_of_keyframe_tracks() const { return keyframe_tracks_.size(); }
...
void NodeInput::KeyframeValueChanged() { emit_range_affected_by_keyframe(static_cast<NodeKeyframe *>(sender())); }
...
void NodeInput::emit_time_range(const TimeRange &range) { emit ValueChanged(range); }
...
bool NodeInput::is_keyframing() const { return keyframing_; }
...
bool NodeInput::is_keyframable() const { return keyframable_; }

QVariant NodeInput::get_standard_value() const { return combine_track_values_into_normal_value(standard_value_); }

const QVector<QVariant> &NodeInput::get_split_standard_value() const { return standard_value_; }
...
const QVector<NodeInput::KeyframeTrack> &NodeInput::keyframe_tracks() const { return keyframe_tracks_; }

void NodeInput::set_is_keyframable(bool k) { keyframable_ = k; }
...
void NodeInput::CopyValues(NodeInput *source, NodeInput *dest, bool include_connections) {
  ...
  for (int i = 0; i < source->keyframe_tracks_.size(); i++) {
    ...
    foreach (NodeKeyframePtr key, source->keyframe_tracks_.at(i)) { dest->keyframe_tracks_[i].append(key->copy()); }
  }
...
QVariant NodeInput::get_property(const QString &key) const { return properties_.value(key); }

bool NodeInput::has_property(const QString &key) const { return properties_.contains(key); }

const QHash<QString, QVariant> &NodeInput::properties() const { return properties_; }
...
QStringList NodeInput::get_combobox_strings() const { return get_property(QStringLiteral("combo_str")).toStringList(); }

void NodeInput::set_combobox_strings(const QStringList &strings) { set_property(QStringLiteral("combo_str"), strings); }
