...
MediaInput::MediaInput() : connected_footage_(nullptr) {
...
StreamPtr MediaInput::footage() { return footage_input_->get_standard_value().value<StreamPtr>(); }

void MediaInput::SetFootage(StreamPtr f) { footage_input_->set_standard_value(QVariant::fromValue(f)); }

void MediaInput::Retranslate() { footage_input_->set_name(tr("Footage")); }

NodeValueTable MediaInput::Value(const NodeValueDatabase &value) const {
  NodeValueTable table = value.Merge();

  if (connected_footage_) {
    rational media_duration =
        Timecode::timestamp_to_time(connected_footage_->duration(), connected_footage_->timebase());
...
void MediaInput::FootageParametersChanged() { InvalidateCache(0, RATIONAL_MAX, footage_input_); }
