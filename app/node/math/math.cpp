...
QString MathNode::Description() const { return tr("Perform a mathematical operation between two."); }
...
QString MathNode::ShaderFragmentCode(const NodeValueDatabase &input) const {
  ...
  return QStringLiteral(
             ...
             "  gl_FragColor = %5 + %6;\n"
             "}\n")
      .arg(GetShaderUniformType(type_a), GetShaderUniformType(type_b), param_a_in_->id(), param_b_in_->id(),
           GetShaderVariableCall(param_a_in_->id(), type_a), GetShaderVariableCall(param_b_in_->id(), type_b));
}
    ...
    case kPairNumberNumber: {
      if (val_a.type() == NodeParam::kRational && val_b.type() == NodeParam::kRational) {
        ...
        output.Push(NodeParam::kRational,
                    QVariant::fromValue(val_a.data().value<rational>() + val_b.data().value<rational>()));
      } else {
...
MathNode::Operation MathNode::GetOperation() const { return kOpAdd; }
...
MathNode::Pairing MathNode::GetMostLikelyPairing(const QVector<int> &a, const QVector<int> &b) {
  ...
  for (int i = 0; i < likelihoods.size(); i++) {
    ...
    if (likelihoods.at(i) > -1) {
      if (pairing == kPairNone || likelihoods.at(i) > likelihoods.at(pairing)) {
