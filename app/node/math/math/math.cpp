...
Node *MathNode::copy() const { return new MathNode(); }

QString MathNode::Name() const { return tr("Math"); }

QString MathNode::id() const { return QStringLiteral("org.olivevideoeditor.Olive.math"); }

QString MathNode::Category() const { return tr("Math"); }
...
void MathNode::Retranslate() {
  ...
  QStringList operations = {tr("Add"), tr("Subtract"), tr("Multiply"), tr("Divide"), QString(), tr("Power")};
...
QString MathNode::ShaderFragmentCode(const NodeValueDatabase &input) const {
  ...
  return QStringLiteral(
             ...
             "    fragColor = %5;\n"
             "}\n")
      .arg(GetShaderUniformType(type_a), GetShaderUniformType(type_b), param_a_in_->id(), param_b_in_->id(), operation);
}
...
NodeValue MathNode::InputValueFromTable(NodeInput *input, NodeValueDatabase &db, bool take) const {
  if (input == param_a_in_ || input == param_b_in_) {
    ...
    NodeValue v = (input == param_a_in_) ? calc.GetMostLikelyValueA() : calc.GetMostLikelyValueB();
...
NodeValueTable MathNode::Value(NodeValueDatabase &value) const {
  ...
  if (!calc.FoundMostLikelyPairing() || calc.GetMostLikelyPairing() == kPairSampleNumber ||
      calc.GetMostLikelyPairing() == kPairTextureTexture || calc.GetMostLikelyPairing() == kPairTextureNumber ||
      calc.GetMostLikelyPairing() == kPairTextureColor || calc.GetMostLikelyPairing() == kPairTextureMatrix) {
    return value.Merge();
  }
  ...
  switch (calc.GetMostLikelyPairing()) {
    case kPairNumberNumber: {
      if (val_a.type() == NodeParam::kRational && val_b.type() == NodeParam::kRational && GetOperation() != kOpPower) {
        // Preserve rationals
        output.Push(NodeParam::kRational, QVariant::fromValue(PerformAddSubMultDiv<rational, rational>(
                                              val_a.data().value<rational>(), val_b.data().value<rational>())));
      } else {
        output.Push(NodeParam::kFloat, PerformAll<float, float>(RetrieveNumber(val_a), RetrieveNumber(val_b)));
      }
      break;
    }
    ...
    case kPairMatrixVec: {
      QMatrix4x4 matrix =
          (val_a.type() == NodeParam::kMatrix) ? val_a.data().value<QMatrix4x4>() : val_b.data().value<QMatrix4x4>();
      QVector4D vec = (val_a.type() == NodeParam::kMatrix) ? RetrieveVector(val_b) : RetrieveVector(val_a);

      // Only valid operation is multiply
      PushVector(&output, qMax(val_a.type(), val_b.type()), PerformMult<QVector4D, QMatrix4x4>(vec, matrix));
      break;
    }
    ...
    case kPairSampleSample: {
      ...
      if (max_samples > min_samples) {
        ...
        for (int i = 0; i < mixed_samples->audio_params().channel_count(); i++) {
          memset(mixed_samples->data()[i] + min_samples * sizeof(float), 0, remainder * sizeof(float));
        }
      }
...
void MathNode::ProcessSamples(const NodeValueDatabase &values, const AudioRenderingParams &params,
                              const SampleBufferPtr input, SampleBufferPtr output, int index) const {
...
NodeInput *MathNode::param_a_in() const { return param_a_in_; }

NodeInput *MathNode::param_b_in() const { return param_b_in_; }

MathNode::Operation MathNode::GetOperation() const {
  return static_cast<Operation>(method_in_->get_standard_value().toInt());
}

void MathNode::SetOperation(MathNode::Operation o) { method_in_->set_standard_value(o); }
...
MathNode::Pairing MathNode::PairingCalculator::GetMostLikelyPairing() const { return most_likely_pairing_; }

const NodeValue &MathNode::PairingCalculator::GetMostLikelyValueA() const { return most_likely_value_a_; }

const NodeValue &MathNode::PairingCalculator::GetMostLikelyValueB() const { return most_likely_value_b_; }
