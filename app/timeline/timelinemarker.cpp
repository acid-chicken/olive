...
TimelineMarker::TimelineMarker(const TimeRange &time, const QString &name, QObject *parent)
    : QObject(parent), time_(time), name_(name) {}

const TimeRange &TimelineMarker::time() const { return time_; }
...
const QString &TimelineMarker::name() const { return name_; }
...
TimelineMarkerList::~TimelineMarkerList() { qDeleteAll(markers_); }
...
const QList<TimelineMarker *> &TimelineMarkerList::list() const { return markers_; }
