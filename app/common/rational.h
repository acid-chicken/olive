...
class rational {
 public:
  // constructors
  rational(const intType &numerator = 0) : numer(numerator), denom(1) {
    if (numer == 0) denom = 0;
  }

  rational(const intType &numerator, const intType &denominator) : numer(numerator), denom(denominator) {
    validateConstructor();
  }
