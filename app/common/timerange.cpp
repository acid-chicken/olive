...
TimeRange TimeRange::CombineWith(const TimeRange &a) const { return Combine(a, *this); }
...
TimeRange TimeRange::Combine(const TimeRange &a, const TimeRange &b) {
  return TimeRange(qMin(a.in(), b.in()), qMax(a.out(), b.out()));
}
...
TimeRangeList TimeRangeList::Intersects(const TimeRange &range) {
  ...
  for (int i = 0; i < size(); i++) {
    ...
    if (compare.out() <= range.in() || compare.in() >= range.out()) {
      ...
      TimeRange cropped(qMax(range.in(), compare.in()), qMin(range.out(), compare.out()));
...
uint qHash(const TimeRange &r, uint seed) { return qHash(r.in(), seed) ^ qHash(r.out(), seed); }
