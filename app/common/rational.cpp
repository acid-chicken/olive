...
rational::rational(const AVRational &r) : numer(r.num), denom(r.den) { validateConstructor(); }
...
void rational::print(std::ostream &out) const { out << this->numer << "/" << this->denom; }
...
void rational::reduce() {
  ...
  if (denom != 0 && numer != 0) d = gcd(numer, denom);
...
rational rational::flipped() const { return rational(denom, numer); }

bool rational::isNull() const { return denominator() == 0; }

const intType &rational::numerator() const { return numer; }

const intType &rational::denominator() const { return denom; }
...
const rational &rational::operator+=(const rational &rhs) {
  if (numer * denom == intType(0) && rhs.numer * rhs.denom == intType(0)) {
    numer = intType(0);
    denom = intType(0);
  } else if (numer * denom != intType(0) && rhs.numer * rhs.denom == intType(0)) {
  } else if (numer * denom == intType(0) && rhs.numer * rhs.denom != intType(0)) {
    numer = rhs.numer;
    denom = rhs.denom;
  } else {
...
bool rational::operator==(const rational &rhs) const { return (numer == rhs.numer && denom == rhs.denom); }
...
rational rational::operator-() const { return rational(numer, -denom); }

bool rational::operator!() const { return !numer; }
