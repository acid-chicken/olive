...
OIIODecoder::OIIODecoder() : image_(nullptr), buffer_(nullptr) {}
  ...
  if (GetImageSequenceDigitCount(f->filename()) > 0) {
    ...
    if (QFileInfo::exists(TransformImageSequenceFileName(f->filename(), ind - 1)) ||
      ...
      QMetaObject::invokeMethod(Core::instance(), "ConfirmImageSequence", Qt::BlockingQueuedConnection,
                                Q_RETURN_ARG(bool, is_sequence_), Q_ARG(QString, f->filename()));
    }
  }
...
FramePtr OIIODecoder::RetrieveVideo(const rational &timecode, const int &divider, bool /*use_proxies*/) {
  ...
  frame->set_video_params(
      VideoRenderingParams(buffer_->spec().width / divider, buffer_->spec().height / divider, pix_fmt_));
  frame->allocate();
  ...
  if (divider == 1) {
    ...
    OIIO::ImageBuf dst(
        OIIO::ImageSpec(frame->width(), frame->height(), buffer_->spec().nchannels, buffer_->spec().format));
...
QString OIIODecoder::GetIndexFilename() const { return QString(); }
...
#else
  buf->set_pixels(OIIO::ROI(), buf->spec().format, frame->data(), OIIO::AutoStride, frame->linesize_bytes());
#endif
}
...
#else
  buffer_ =
      new OIIO::ImageBuf(OIIO::ImageSpec(spec.width, spec.height, spec.nchannels, type), OIIO::InitializePixels::No);
#endif
