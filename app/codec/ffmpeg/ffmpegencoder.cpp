...
void FFmpegEncoder::WriteAudio(const AudioRenderingParams& pcm_info, const QString& pcm_filename) {
  ...
  if (pcm.open(QFile::ReadOnly)) {
    ...
    SwrContext* swr_ctx = swr_alloc_set_opts(
        nullptr, static_cast<int64_t>(audio_codec_ctx_->channel_layout), audio_codec_ctx_->sample_fmt,
        audio_codec_ctx_->sample_rate, static_cast<int64_t>(pcm_info.channel_layout()),
        FFmpegCommon::GetFFmpegSampleFormat(pcm_info.format()), pcm_info.sample_rate(), 0, nullptr);
    ...
    while (!pcm.atEnd()) {
      ...
      int converted = swr_convert(swr_ctx, frame->data, maximum_frame_samples,
                                  reinterpret_cast<const uint8_t**>(&input_data_array), samples_needed);
...
bool FFmpegEncoder::OpenInternal() {
  ...
  if (params().video_enabled()) {
    ...
    video_scale_ctx_ = sws_getContext(params().video_params().width(), params().video_params().height(), src_pix_fmt,
                                      params().video_params().width(), params().video_params().height(),
                                      encoder_pix_fmt, 0, nullptr, nullptr, nullptr);
  }
...
void FFmpegEncoder::WriteInternal(FramePtr frame) {
  ...
  error_code = sws_scale(video_scale_ctx_, reinterpret_cast<const uint8_t**>(&input_data), &input_linesize, 0,
                         frame->height(), encoded_frame->data, encoded_frame->linesize);
  if (error_code < 0) {
...
void FFmpegEncoder::FFmpegError(const char* context, int error_code) {
  ...
  Error(QStringLiteral("%1 for %2 - %3 %4").arg(context, params().filename(), QString::number(error_code), err));
}
