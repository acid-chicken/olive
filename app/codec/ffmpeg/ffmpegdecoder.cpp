...
FFmpegDecoder::FFmpegDecoder()
    : fmt_ctx_(nullptr), codec_ctx_(nullptr), scale_ctx_(nullptr), pkt_(nullptr), frame_(nullptr), opts_(nullptr) {}
...
bool FFmpegDecoder::Open() {
  ...
  if (codec == nullptr) {
    Error(QStringLiteral("Failed to find appropriate decoder for this codec (%1:%2 - %3)")
              .arg(stream()->footage()->filename(), QString::number(avstream_->index),
  ...
  if (codec_ctx_ == nullptr) {
    Error(QStringLiteral("Failed to allocate codec context (%1 :: %2)")
              .arg(stream()->footage()->filename(), stream()->index()));
    return false;
  }
  ...
  if (avstream_->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
    ...
    scale_ctx_ = sws_getContext(avstream_->codecpar->width, avstream_->codecpar->height,
                                static_cast<AVPixelFormat>(avstream_->codecpar->format), avstream_->codecpar->width,
                                avstream_->codecpar->height, ideal_pix_fmt_, 0, nullptr, nullptr, nullptr);
...
FramePtr FFmpegDecoder::RetrieveVideo(const rational& timecode) {
  ...
  if (!got_frame) {
    ...
    if (compressed_frame.exists() && compressed_frame.size() > 0 && compressed_frame.open(QFile::ReadOnly)) {
      ...
      av_image_fill_arrays(input_data, input_linesize, reinterpret_cast<uint8_t*>(frame_loader.data()),
                           static_cast<AVPixelFormat>(avstream_->codecpar->format), avstream_->codecpar->width,
                           avstream_->codecpar->height, 1);
    ...
    while (true) {
      ...
      if (frame_->pts == target_ts) {
        ...
        if (save_frame.open(QFile::WriteOnly)) {
          ...
          int cached_buffer_sz =
              av_image_get_buffer_size(static_cast<AVPixelFormat>(frame_->format), frame_->width, frame_->height, 1);

          QByteArray cached_frame(cached_buffer_sz, Qt::Uninitialized);

          av_image_copy_to_buffer(reinterpret_cast<uint8_t*>(cached_frame.data()), cached_frame.size(), frame_->data,
                                  frame_->linesize, static_cast<AVPixelFormat>(frame_->format), frame_->width,
                                  frame_->height, 1);
  ...
  if (got_frame) {
    ...
    sws_scale(scale_ctx_, input_data, input_linesize, 0, avstream_->codecpar->height, &output_data, &output_linesize);
...
FramePtr FFmpegDecoder::RetrieveAudio(const rational& timecode, const rational& length,
  ...
  if (input.open()) {
    ...
    input.read(input_params.time_to_bytes(timecode), audio_frame->data(), audio_frame->allocated_size());
...
QString FFmpegDecoder::id() { return "ffmpeg"; }
...
void FFmpegDecoder::Conform(const AudioRenderingParams& params) {
  ...
  if (input.open()) {
    ...
    SwrContext* resampler = swr_alloc_set_opts(
        nullptr, static_cast<int64_t>(params.channel_layout()), FFmpegCommon::GetFFmpegSampleFormat(params.format()),
        params.sample_rate(), static_cast<int64_t>(input.params().channel_layout()),
        FFmpegCommon::GetFFmpegSampleFormat(input.params().format()), input.params().sample_rate(), 0, nullptr);
...
void FFmpegDecoder::ConformInternal(SwrContext* resampler, WaveOutput* output, const char* in_data,
                                    int in_sample_count) {
...
bool FFmpegDecoder::Probe(Footage* f) {
  ...
  if (need_manual_duration) {
    ...
    for (int i = 1; i < f->stream_count(); i++) {
      int64_t new_dur =
          av_rescale_q(duration, f->stream(0)->timebase().toAVRational(), f->stream(i)->timebase().toAVRational());
  ...
  switch (avstream_->codecpar->codec_type) {
    case AVMEDIA_TYPE_VIDEO: {
      ...
      if (index_file.open(QFile::ReadOnly)) {
        ...
        index_file.read(reinterpret_cast<char*>(frame_index_.data()), index_file.size());
...
void FFmpegDecoder::IndexAudio(AVPacket* pkt, AVFrame* frame) {
  ...
  if (av_sample_fmt_is_planar(src_sample_fmt)) {
    ...
    resampler =
        swr_alloc_set_opts(nullptr, static_cast<int64_t>(avstream_->codecpar->channel_layout), dst_sample_fmt,
                           avstream_->codecpar->sample_rate, static_cast<int64_t>(avstream_->codecpar->channel_layout),
                           src_sample_fmt, avstream_->codecpar->sample_rate, 0, nullptr);
  } else {
    dst_sample_fmt = src_sample_fmt;
  }

  WaveOutput wave_out(GetIndexFilename(), AudioRenderingParams(avstream_->codecpar->sample_rate, channel_layout,
  ...
  if (wave_out.open()) {
    while (true) {
      ...
      if (ret < 0) {
        ...
        int buffer_sz =
            av_samples_get_buffer_size(nullptr, avstream_->codecpar->channels, data_frame->nb_samples, dst_sample_fmt,
