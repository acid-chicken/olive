...
FFmpegDecoder::FFmpegDecoder()
  ...
  connect(this, &FFmpegDecoder::ConsumedMemory, MemoryManager::instance(), &MemoryManager::ConsumedMemory,
          Qt::DirectConnection);
  connect(MemoryManager::instance(), &MemoryManager::FreeMemory, this, &FFmpegDecoder::FreeMemory,
          Qt::DirectConnection);
...
FramePtr FFmpegDecoder::RetrieveVideo(const rational& timecode) {
  ...
  if (!cached_frames_.isEmpty()) {
    ...
    } else if (target_ts >= cached_frames_.first()->pts && target_ts <= cached_frames_.last()->pts) {
      ...
      for (int i = 0; i < cached_frames_.size(); i++) {
        ...
        if (this_frame->pts == target_ts  // Test for an exact match
            || (i < cached_frames_.size() - 1 &&
                cached_frames_.at(i + 1)->pts > target_ts)) {  // Or for this frame to be the "closest"
  ...
  */
  ...
  if (!got_frame) {
    ...
    if (cached_frames_.isEmpty() || target_ts < cached_frames_.first()->pts ||
