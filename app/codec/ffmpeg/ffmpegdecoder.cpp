...
FramePtr FFmpegDecoder::RetrieveVideo(const rational& timecode) {
      ...
      for (int i = 0; i < cached_frames_.size(); i++) {
        ...
        if (this_frame->native_timestamp() == target_ts  // Test for an exact match
            || (i < cached_frames_.size() - 1 &&
                cached_frames_.at(i + 1)->native_timestamp() > target_ts)) {  // Or for this frame to be the "closest"
  ...
  */
  ...
  if (!return_frame) {
    ...
    if (cached_frames_.isEmpty() || target_ts < cached_frames_.first()->native_timestamp() ||
    ...
    while (true) {
      ...
      if (ret == AVERROR_EOF) {
        ...
        Frame* working_frame_converted =
            cached_frames_.append(VideoRenderingParams(avstream_->codecpar->width, avstream_->codecpar->height,
                                                       avstream_->time_base, native_pix_fmt_, RenderMode::kOffline));
        ...
        int output_linesize = working_frame_converted->width() * PixelFormat::ChannelCount(native_pix_fmt_) *
                              PixelFormat::BytesPerChannel(native_pix_fmt_);

        sws_scale(scale_ctx_, working_frame->data, working_frame->linesize, 0, avstream_->codecpar->height,
  ...
  if (stream()->type() == Stream::kAudio) {
    if (QFileInfo::exists(GetIndexFilename())) {
      ...
      if (input.open()) {
        ...
        std::static_pointer_cast<AudioStream>(stream())->set_index_length(
            input.params().bytes_to_time(input.data_length()));
...
void FFmpegDecoder::UnconditionalAudioIndex(const QAtomicInt* cancelled) {
  ...
  AudioRenderingParams wave_params(avstream_->codecpar->sample_rate, channel_layout,
