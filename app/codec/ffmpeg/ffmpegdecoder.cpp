  ...
  if (!return_frame) {
    ...
    while (true) {
      ...
      if (ret == AVERROR_EOF) {
        ...
        Frame* working_frame_converted = cached_frames_.append(
            VideoRenderingParams(avstream_->codecpar->width / divider, avstream_->codecpar->height / divider,
...
void FFmpegDecoder::SetupScaler(const int& divider) {
  scale_ctx_ = sws_getContext(avstream_->codecpar->width, avstream_->codecpar->height,
                              static_cast<AVPixelFormat>(avstream_->codecpar->format),
                              avstream_->codecpar->width / divider, avstream_->codecpar->height / divider,
                              ideal_pix_fmt_, SWS_FAST_BILINEAR, nullptr, nullptr, nullptr);
