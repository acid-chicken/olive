...
FramePtr FFmpegDecoder::RetrieveVideo(const rational& timecode, const int& divider) {
  ...
  if (return_frame) {
    ...
    copy->set_video_params(VideoRenderingParams(GetScaledDimension(vs->width(), divider),
                                                GetScaledDimension(vs->height(), divider), native_pix_fmt_));
...
void FFmpegDecoder::UnconditionalAudioIndex(const QAtomicInt* cancelled) {
  ...
  if (av_sample_fmt_is_planar(src_sample_fmt)) {
    ...
    resampler = swr_alloc_set_opts(nullptr, static_cast<int64_t>(index_instance.stream()->codecpar->channel_layout),
                                   dst_sample_fmt, index_instance.stream()->codecpar->sample_rate,
                                   static_cast<int64_t>(index_instance.stream()->codecpar->channel_layout),
                                   src_sample_fmt, index_instance.stream()->codecpar->sample_rate, 0, nullptr);
  ...
  AudioRenderingParams wave_params(index_instance.stream()->codecpar->sample_rate, channel_layout,
