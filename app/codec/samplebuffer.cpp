...
SampleBuffer::SampleBuffer() : sample_count_per_channel_(0), data_(nullptr) {}

SampleBuffer::~SampleBuffer() { destroy(); }

SampleBufferPtr SampleBuffer::Create() { return std::make_shared<SampleBuffer>(); }
...
const AudioRenderingParams &SampleBuffer::audio_params() const { return audio_params_; }
...
const int &SampleBuffer::sample_count_per_channel() const { return sample_count_per_channel_; }
...
float **SampleBuffer::data() { return data_; }

const float **SampleBuffer::const_data() const { return const_cast<const float **>(data_); }

float *SampleBuffer::channel_data(int channel) { return data_[channel]; }

bool SampleBuffer::is_allocated() const { return data_; }
...
void SampleBuffer::destroy() { destroy_sample_buffer(&data_, audio_params_.channel_count()); }
...
void SampleBuffer::fill(const float &f) { fill(f, 0, sample_count_per_channel_); }
...
void SampleBuffer::set(const float **data, int sample_length) { set(data, 0, sample_length); }
