...
class Decoder : public QObject {
 ...
 public:
   ...
   */
  virtual SampleBufferPtr RetrieveAudio(const rational& timecode, const rational& length,
   ...
   * the application. This function will take a Footage file and manually pass it through the available Decoders'
   * Probe() functions until one indicates that it can decode this file. That Decoder will then dump information about
   * the file into the Footage object for use throughout the program.
   *
   * Probing may be a lengthy process and it's recommended to run this in a separate thread.
   *
   * @param f
   *
   * A Footage object with a valid filename. If the Footage does not have a valid filename (e.g. is empty or file
   * doesn't exist), this function will return FALSE.
   *
   * @return
   *
