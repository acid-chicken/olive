...
WaveInput::WaveInput(const QString &f) : file_(f) {}

WaveInput::~WaveInput() { close(); }
...
bool WaveInput::is_open() const { return file_.isOpen(); }
...
bool WaveInput::seek(qint64 pos) { return file_.seek(data_position_ + qMin(pos, static_cast<qint64>(data_size_))); }

bool WaveInput::at_end() const { return file_.pos() == (data_position_ + data_size_); }

const AudioRenderingParams &WaveInput::params() const { return params_; }
...
int WaveInput::sample_count() const { return params_.bytes_to_samples(static_cast<int>(data_size_)); }
...
qint64 WaveInput::calculate_max_read() const { return data_size_ - (file_.pos() - data_position_); }
