...
AudioRenderBackend::AudioRenderBackend(QObject *parent) : RenderBackend(parent) {
...
void AudioRenderBackend::SetParameters(const AudioRenderingParams &params) {
  ...
  //        recompile signal
  foreach (RenderWorker *worker, processors_) { static_cast<AudioRenderWorker *>(worker)->SetParameters(params_); }
...
void AudioRenderBackend::DecompileInternal() { copy_map_.clear(); }
...
const AudioRenderingParams &AudioRenderBackend::params() { return params_; }

NodeInput *AudioRenderBackend::GetDependentInput() { return viewer_node()->samples_input(); }
...
bool AudioRenderBackend::CanRender() { return params_.is_valid(); }
...
void AudioRenderBackend::ConnectWorkerToThis(RenderWorker *worker) {
  ...
  connect(arw, &AudioRenderWorker::ConformUnavailable, this, &AudioRenderBackend::ConformUnavailable,
          Qt::QueuedConnection);
}
...
void AudioRenderBackend::ConformUnavailable(StreamPtr stream, const TimeRange &range, const rational &stream_time,
...
void AudioRenderBackend::ConformUpdated(Stream *stream, const AudioRenderingParams &params) {
  for (int i = 0; i < conform_wait_info_.size(); i++) {
    ...
    if (info.stream.get() == stream && info.params == params) {
...
bool AudioRenderBackend::ConformWaitInfo::operator==(const AudioRenderBackend::ConformWaitInfo &rhs) const {
  return rhs.params == params && rhs.stream == stream && rhs.stream_time == stream_time &&
