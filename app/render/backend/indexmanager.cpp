...
void IndexManager::CreateInstance() { instance_ = new IndexManager(); }

IndexManager* IndexManager::instance() { return instance_; }
...
void IndexManager::StartConformingStream(AudioStreamPtr stream, AudioRenderingParams params) {
  ...
  connect(stream.get(), &AudioStream::ConformAppended, this, &IndexManager::StreamConformAppendedEvent,
          Qt::QueuedConnection);
...
void IndexManager::StreamIndexUpdatedEvent() { emit StreamIndexUpdated(static_cast<Stream*>(sender())); }
