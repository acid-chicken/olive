...
RenderBackend::RenderBackend(QObject* parent)
    : QObject(parent), started_(false), viewer_node_(nullptr), copied_viewer_node_(nullptr) {
...
void RenderBackend::Close() {
  ...
  for (int i = 0; i < processors_.size(); i++) {
    ...
    QMetaObject::invokeMethod(processors_.at(i), "Close", Qt::QueuedConnection);
...
const QString& RenderBackend::GetError() const { return error_; }
...
bool RenderBackend::IsInitiated() { return started_; }

void RenderBackend::RegenerateCacheID() {
  QCryptographicHash hash(QCryptographicHash::Sha1);

  if (!viewer_node_ || !GenerateCacheIDInternal(hash)) {
    cache_id_.clear();
...
bool RenderBackend::InitInternal() { return true; }

void RenderBackend::CloseInternal() {}

bool RenderBackend::CanRender() { return true; }

TimeRange RenderBackend::PopNextFrameFromQueue() { return cache_queue_.takeFirst(); }
...
void RenderBackend::SetError(const QString& error) { error_ = error; }

void RenderBackend::ConnectViewer(ViewerOutput* node) { Q_UNUSED(node) }

void RenderBackend::DisconnectViewer(ViewerOutput* node) { Q_UNUSED(node) }
...
void RenderBackend::CacheNext() {
  ...
  if (!ViewerIsConnected() || !CanRender() || !Init()) {
    return;
  }
  ...
  foreach (RenderWorker* worker, processors_) {
    ...
    if (!WorkerIsBusy(worker)) {
      ...
      NodeDependency dep = NodeDependency(node_connected_to_viewer, cache_frame);
      ...
      QMetaObject::invokeMethod(worker, "Render", Qt::QueuedConnection, OLIVE_NS_ARG(NodeDependency, dep),
                                Q_ARG(qint64, job_time));
    }
  }
}

ViewerOutput* RenderBackend::viewer_node() const { return copied_viewer_node_; }
...
void RenderBackend::CancelQueue() {
  ...
  for (int i = 0; i < processor_busy_state_.size(); i++) {
    if (processor_busy_state_.at(i)) busy++;
  }
...
void RenderBackend::InvalidateCache(const TimeRange& range, NodeInput* from) {
  ...
  qDebug() << "Cache invalidated between" << start_range_adj.toDouble() << "and" << end_range_adj.toDouble();
...
bool RenderBackend::ViewerIsConnected() const { return viewer_node_; }

const QString& RenderBackend::cache_id() const { return cache_id_; }
...
void RenderBackend::CopyNodeInputValue(NodeInput* input) {
  ...
  NodeInput::CopyValues(input, our_copy, false);
  ...
  if (input->IsArray()) {
    foreach (NodeInput* i, static_cast<NodeInputArray*>(input)->sub_params()) { CopyNodeInputValue(i); }
  }
}
...
void RenderBackend::CopyNodeMakeConnection(NodeInput* src_input, NodeInput* dst_input) {
  ...
  if (src_input->IsConnected()) {
    ...
    NodeParam::ConnectEdge(corresponding_output, dst_input);
  }
}
...
const QVector<QThread*>& RenderBackend::threads() { return threads_; }
...
void RenderBackend::CacheIDChangedEvent(const QString& id) { Q_UNUSED(id) }
...
void RenderBackend::InitWorkers() {
  for (int i = 0; i < processors_.size(); i++) {
    ...
    connect(processor, &RenderWorker::CompletedCache, cancel_dialog_, &RenderCancelDialog::WorkerDone,
            Qt::QueuedConnection);
    connect(processor, &RenderWorker::FootageUnavailable, this, &RenderBackend::FootageUnavailable,
            Qt::QueuedConnection);
...
void RenderBackend::FootageUnavailable(StreamPtr stream, Decoder::RetrieveState state, const TimeRange& range,
                                       const rational& stream_time) {
  if (state == Decoder::kFailedToOpen) {
    qWarning() << "For range" << range.in() << "-" << range.out() << stream->footage()->filename() << "stream"
               << stream->index() << "failed to open";
    ...
    } else if ((stream->type() == Stream::kVideo &&
                std::static_pointer_cast<VideoStream>(stream)->is_frame_index_ready()) ||
    ...
    if (info.stream.get() == stream) {
      ...
      if (stream->type() == Stream::kVideo) {
        ...
        if (video_stream->get_closest_timestamp_in_frame_index(info.stream_time) >= 0) {
          ...
          qDebug() << "Re-ICing video" << info.affected_range.in().toDouble() << "to"
                   << info.affected_range.out().toDouble();
          footage_ready = true;
        }
        ...
        if (audio_stream->index_length() >= info.stream_time) {
          ...
          qDebug() << "Re-ICing audio" << info.affected_range.in().toDouble() << "to"
                   << info.affected_range.out().toDouble();
          footage_ready = true;
        }
      }
...
bool RenderBackend::FootageWaitInfo::operator==(const RenderBackend::FootageWaitInfo& rhs) const {
  return rhs.stream == stream && rhs.stream_time == stream_time && rhs.affected_range == affected_range;
}
