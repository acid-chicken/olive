...
void RenderBackend::CacheNext() {
  ...
  if (!Init() || !ViewerIsConnected() || !CanRender()) {
    return;
  }
  ...
  foreach (RenderWorker *worker, processors_) {
    ...
    if (!WorkerIsBusy(worker)) {
      ...
      QMetaObject::invokeMethod(worker, "Render", Qt::QueuedConnection, Q_ARG(NodeDependency, dep),
...
bool RenderBackend::ViewerIsConnected() const { return viewer_node_ != nullptr; }

const QString &RenderBackend::cache_id() const { return cache_id_; }

void RenderBackend::QueueValueUpdate() { input_update_queued_ = true; }
...
DecoderCache *RenderBackend::decoder_cache() { return &decoder_cache_; }
...
void RenderBackend::QueueRecompile() { recompile_queued_ = true; }
