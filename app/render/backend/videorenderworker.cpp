...
void VideoRenderWorker::TextureToBuffer(const QVariant &texture, void *buffer, int linesize) {
  TextureToBuffer(texture, video_params_.effective_width(), video_params_.effective_height(), QMatrix4x4(), buffer,
                  linesize);
}
...
void VideoRenderWorker::SetFrameGenerationParams(int width, int height, const QMatrix4x4 &matrix) {
  frame_gen_params_ = VideoRenderingParams(width, height, video_params_.time_base(), video_params_.format(),
                                           video_params_.mode(), video_params_.divider());
...
void VideoRenderWorker::Download(const rational &time, QVariant texture, QString filename) {
  if (operating_mode_ & kDownloadOnly) {
    ...
    switch (video_params().format()) {
      ...
      case PixelFormat::PIX_FMT_RGBA16U: {
        ...
        if (out) {
          ...
          out->open(fn_std, OIIO::ImageSpec(video_params().effective_width(), video_params().effective_height(),
      ...
      case PixelFormat::PIX_FMT_RGBA32F: {
        ...
        Imf::Header header(video_params().effective_width(), video_params().effective_height());
    ...
    if (frame_gen_params_.is_valid()) {
      ...
      frame->set_video_params(VideoRenderingParams(video_params_.effective_width(), video_params_.effective_height(),
    ...
    if (texture.isNull()) {
      ...
      TextureToBuffer(texture, frame->width(), frame->height(), frame_gen_mat_, frame->data(),
...
void VideoRenderWorker::ResizeDownloadBuffer() {
  download_buffer_.resize(PixelFormat::GetBufferSize(video_params_.format(), video_params_.effective_width(),
...
void VideoRenderWorker::ReportUnavailableFootage(StreamPtr stream, Decoder::RetrieveState state,
                                                 const rational &stream_time) {
  emit FootageUnavailable(stream, state, TimeRange(CurrentPath().in(), CurrentPath().in() + video_params().time_base()),
                          stream_time);
}
