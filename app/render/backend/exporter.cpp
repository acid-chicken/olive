...
Exporter::Exporter(ViewerOutput *viewer, Encoder *encoder, QObject *parent)
    : QObject(parent),
...
void Exporter::EnableVideo(const VideoRenderingParams &video_params, const QMatrix4x4 &transform,
...
void Exporter::StartExporting() {
  ...
  if (!video_done_) {
    ...
    video_backend_->SetParameters(VideoRenderingParams(viewer_node_->video_params().width(),
                                                       viewer_node_->video_params().height(), video_params_.time_base(),
                                                       video_params_.format(), video_params_.mode()));
...
void Exporter::ExportFailed() { emit ExportEnded(); }
...
void Exporter::EncodeFrame() {
  while (cached_frames_.contains(waiting_for_frame_)) {
    ...
    QMetaObject::invokeMethod(encoder_, "WriteFrame", Qt::QueuedConnection, Q_ARG(FramePtr, frame));
...
void Exporter::VideoHashesComplete() {
  ...
  foreach (const TimeRange &range, ranges) { video_backend_->InvalidateCache(range); }
}
