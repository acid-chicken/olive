...
void OpenGLProxy::FrameToValue(DecoderPtr decoder, StreamPtr stream, const TimeRange &range, NodeValueTable *table) {
  ...
  // Set up OCIO context
  QString colorspace_match =
      QStringLiteral("%1:%2").arg(video_stream->footage()->project()->ocio_config(), video_stream->colorspace());
  ...
  if (!footage_tex_ref) {
    ...
    if (ocio_method == ColorManager::kOCIOFast) {
      ...
      if (frame->sample_aspect_ratio() != 1 && frame->sample_aspect_ratio() != 0) {
        ...
        footage_params = VideoRenderingParams(new_width, new_height, footage_params.time_base(),
                                              footage_params.format(), footage_params.mode());
      }
...
void OpenGLProxy::TextureToBuffer(const QVariant &tex_in, void *buffer) {
  ...
  f->glReadPixels(0, 0, video_params_.effective_width(), video_params_.effective_height(),
