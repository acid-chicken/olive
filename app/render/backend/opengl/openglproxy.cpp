  ...
  if (!footage_tex_ref) {
    ...
    if (ocio_method == ColorManager::kOCIOFast) {
      ...
      if (frame->sample_aspect_ratio() != 1 && frame->sample_aspect_ratio() != 0) {
        ...
        footage_params = VideoRenderingParams(new_width, new_height, footage_params.format());
      }

      VideoRenderingParams dest_params(footage_params.width(), footage_params.height(), video_params_.format());
