...
OpenGLProxy::~OpenGLProxy() { surface_.destroy(); }
...
void OpenGLProxy::FrameToValue(StreamPtr stream, FramePtr frame, NodeValueTable *table) {
  ...
  OpenGLColorProcessorPtr color_processor =
      std::static_pointer_cast<OpenGLColorProcessor>(color_cache_.Get(video_stream->colorspace()));
...
void OpenGLProxy::RunNodeAccelerated(const Node *node, const TimeRange &range, const NodeValueDatabase &input_params,
...
void OpenGLProxy::TextureToBuffer(const QVariant &tex_in, QByteArray &buffer) {
  ...
  f->glReadPixels(0, 0, video_params_.effective_width(), video_params_.effective_height(), format_info.pixel_format,
