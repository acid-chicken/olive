...
OpenGLProxy::OpenGLProxy(QObject *parent) : QObject(parent), ctx_(nullptr), functions_(nullptr) { surface_.create(); }
...
void OpenGLProxy::FrameToValue(FramePtr frame, StreamPtr stream, NodeValueTable *table) {
  ...
  // Set up OCIO context
  QString colorspace_match = QStringLiteral("%1:%2").arg(
      video_stream->footage()->project()->color_manager()->GetConfigFilename(), video_stream->colorspace());
  ...
  if (!footage_tex_ref) {
    OpenGLColorProcessorPtr color_processor =
        std::static_pointer_cast<OpenGLColorProcessor>(color_cache_.Get(colorspace_match));

    if (!color_processor) {
      color_processor =
          OpenGLColorProcessor::Create(video_stream->footage()->project()->color_manager(), video_stream->colorspace(),
    ...
    if (ocio_method == ColorManager::kOCIOAccurate) {
      ...
      frame = PixelFormat::ConvertPixelFormat(frame,
                                              has_alpha ? PixelFormat::PIX_FMT_RGBA32F : PixelFormat::PIX_FMT_RGB32F);
    ...
    if (ocio_method == ColorManager::kOCIOFast) {
      ...
      if (frame->sample_aspect_ratio() != 1 && frame->sample_aspect_ratio() != 0) {
        ...
        frame_params = VideoRenderingParams(new_width, new_height, frame_params.format(), frame_params.divider());
      }

      VideoRenderingParams dest_params(frame_params.width(), frame_params.height(), video_params_.format(),
...
void OpenGLProxy::RunNodeAccelerated(const Node *node, const TimeRange &range, NodeValueDatabase &input_params,
  ...
  // Lock the shader so no other thread interferes as we set parameters and draw (and we don't interfere with any
  // others)
  shader->bind();
        ...
        switch (data_type) {
          ...
          case NodeInput::kBuffer: {
            ...
            if (enable_param_location > -1) {
              shader->setUniformValue(enable_param_location, tex_id > 0);
            }
  ...
  shader->setUniformValue("ove_resolution", static_cast<GLfloat>(video_params_.width()),
...
void OpenGLProxy::TextureToBuffer(const QVariant &tex_in, int width, int height, const QMatrix4x4 &matrix, void *buffer,
                                  int linesize) {
  ...
  if (width != texture->texture()->width() || height != texture->texture()->height()) {
    ...
    OpenGLTextureCache::ReferencePtr resized =
        texture_cache_.Get(ctx_, VideoRenderingParams(width, height, texture->texture()->format()));
  ...
  f->glReadPixels(0, 0, width, height, OpenGLRenderFunctions::GetPixelFormat(video_params_.format()),
                  OpenGLRenderFunctions::GetPixelType(video_params_.format()), buffer);
