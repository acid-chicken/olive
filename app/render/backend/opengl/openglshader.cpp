...
OpenGLShaderPtr OpenGLShader::Create() { return std::make_shared<OpenGLShader>(); }
...
OpenGLShaderPtr OpenGLShader::CreateOCIO(QOpenGLContext *ctx, GLuint &lut_texture, OCIO::ConstProcessorRcPtr processor,
  ...
  xf->glTexImage3D(GL_TEXTURE_3D, 0, GL_RGB16F, OCIO_LUT3D_EDGE_SIZE, OCIO_LUT3D_EDGE_SIZE, OCIO_LUT3D_EDGE_SIZE, 0,
                   GL_RGB, GL_FLOAT, &ocio_lut_data[0]);
  ...
  shader_text.append(
      QStringLiteral("#define texture2D texture\n"
  ...
  if (alpha_is_associated) {
    ...
    shader_call = QStringLiteral("%3(%1(%2(col), ove_ociolut));")
                      .arg(ocio_func_name, disassociate_func_name, reassociate_func_name);
...
QString OpenGLShader::CodeDefaultFragment(const QString &function_name, const QString &shader_code) {
  QString frag_code = QStringLiteral(
      "#version 150\n"
      "\n"
      "#ifdef GL_ES\n"
      "precision highp int;\n"
