...
OpenGLWorker::OpenGLWorker(QOpenGLContext *share_ctx, OpenGLShaderCache *shader_cache,
                           OpenGLTextureCache *texture_cache, VideoRenderFrameCache *frame_cache, QObject *parent)
...
OpenGLWorker::~OpenGLWorker() { surface_.destroy(); }
...
void OpenGLWorker::FrameToValue(StreamPtr stream, FramePtr frame, NodeValueTable *table) {
  ...
  OpenGLColorProcessorPtr color_processor =
      std::static_pointer_cast<OpenGLColorProcessor>(color_cache()->Get(video_stream->colorspace()));

  if (!color_processor) {
    // FIXME: We match with the colorspace string, but this won't change if the user sets a new config with a colorspace
    // with the same string
    color_processor =
        OpenGLColorProcessor::CreateOpenGL(video_stream->footage()->project()->color_manager()->GetConfig(),
                                           video_stream->colorspace(), OCIO::ROLE_SCENE_LINEAR);
  ...
  VideoRenderingParams footage_params(frame->width(), frame->height(), stream->timebase(), frame->format(),
                                      video_params().mode());
  ...
  if (ocio_method == ColorManager::kOCIOFast) {
    ...
    if (frame->sample_aspect_ratio() != 1 && frame->sample_aspect_ratio() != 0) {
      ...
      footage_params = VideoRenderingParams(new_width, new_height, footage_params.time_base(), footage_params.format(),
                                            footage_params.mode());
    }
...
void OpenGLWorker::RunNodeAccelerated(const Node *node, const TimeRange &range, const NodeValueDatabase &input_params,
          ...
          case NodeInput::kBuffer: {
            ...
            if (tex_id > 0) {
              ...
              if (res_param_location > -1) {
                shader->setUniformValue(res_param_location, static_cast<GLfloat>(texture->texture()->width()),
  ...
  shader->setUniformValue("ove_resolution", static_cast<GLfloat>(video_params().width()),
...
void OpenGLWorker::TextureToBuffer(const QVariant &tex_in, QByteArray &buffer) {
  ...
  f->glReadPixels(0, 0, texture->texture()->width(), texture->texture()->height(), format_info.pixel_format,
                  format_info.gl_pixel_type, buffer.data());
