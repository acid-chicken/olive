...
OpenGLBackend::OpenGLBackend(QObject* parent) : VideoRenderBackend(parent), master_texture_(nullptr), proxy_(nullptr) {}

OpenGLBackend::~OpenGLBackend() { Close(); }
...
bool OpenGLBackend::InitInternal() {
  ...
  for (int i = 0; i < threads().size(); i++) {
    ...
    connect(processor, &OpenGLWorker::RequestFrameToValue, proxy_, &OpenGLProxy::FrameToValue,
            Qt::BlockingQueuedConnection);
    connect(processor, &OpenGLWorker::RequestTextureToBuffer, proxy_, &OpenGLProxy::TextureToBuffer,
            Qt::BlockingQueuedConnection);
    connect(processor, &OpenGLWorker::RequestRunNodeAccelerated, proxy_, &OpenGLProxy::RunNodeAccelerated,
            Qt::BlockingQueuedConnection);
  }

  // Create master texture (the one sent to the viewer)
  master_texture_ = std::make_shared<OpenGLTexture>();
  master_texture_->Create(QOpenGLContext::currentContext(), params().effective_width(), params().effective_height(),
                          params().format());
...
bool OpenGLBackend::CompileInternal() { return true; }
