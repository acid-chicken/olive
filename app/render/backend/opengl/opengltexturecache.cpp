...
OpenGLTextureCache::~OpenGLTextureCache() {
  foreach (Reference *ref, existing_references_) { ref->ParentKilled(); }
}
...
OpenGLTextureCache::ReferencePtr OpenGLTextureCache::Get(QOpenGLContext *ctx, const VideoRenderingParams &params,
  ...
  for (int i = 0; i < available_textures_.size(); i++) {
    ...
    if (test->width() == params.effective_width() && test->height() == params.effective_height() &&
...
OpenGLTextureCache::Reference::Reference(OpenGLTextureCache *parent, OpenGLTexturePtr texture)
    : parent_(parent), texture_(texture) {}
...
OpenGLTexturePtr OpenGLTextureCache::Reference::texture() { return texture_; }

void OpenGLTextureCache::Reference::ParentKilled() { parent_ = nullptr; }
