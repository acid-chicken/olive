...
RenderWorker::RenderWorker(DecoderCache *decoder_cache, QObject *parent)
    : QObject(parent), started_(false), decoder_cache_(decoder_cache) {}
...
void RenderWorker::RunNodeAccelerated(const Node *node, const TimeRange &range, const NodeValueDatabase &input_params,
...
void RenderWorker::ReportUnavailableFootage(StreamPtr stream, Decoder::RetrieveState state,
...
void RenderWorker::ProcessNodeEvent(const Node *node, const TimeRange &range, const NodeValueDatabase &input_params,
                                    NodeValueTable *output_params) {
