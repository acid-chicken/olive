  ...
  foreach (Block *b, active_blocks) {
    ...
    if (!samples_from_this_block.isEmpty()) {
      ...
      if (b->is_reversed()) {
        ...
        AudioManager::ReverseBuffer(samples_from_this_block.data(), samples_from_this_block.size(),
                                    audio_params_.samples_to_bytes(1));
      }

      int destination_offset = audio_params_.time_to_bytes(range_for_block.in() - range.in());
      int actual_copy_size =
          qMin(audio_params_.time_to_bytes(range_for_block.length()), samples_from_this_block.size());

      if (destination_offset < 0 || destination_offset + actual_copy_size > block_range_buffer.size()) {
        ...
        memcpy(block_range_buffer.data() + destination_offset, samples_from_this_block.data(), actual_copy_size);
      }
    }
