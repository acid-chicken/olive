...
AudioRenderWorker::AudioRenderWorker(DecoderCache *decoder_cache, QHash<Node *, Node *> *copy_map, QObject *parent)
    : RenderWorker(decoder_cache, parent), copy_map_(copy_map) {}

void AudioRenderWorker::SetParameters(const AudioRenderingParams &audio_params) { audio_params_ = audio_params; }
...
NodeValueTable AudioRenderWorker::RenderBlock(const TrackOutput *track, const TimeRange &range) {
  ...
  SampleBufferPtr block_range_buffer =
      SampleBuffer::CreateAllocated(audio_params_, audio_params_.time_to_samples(range.length()));
  ...
  foreach (Block *b, active_blocks) {
    ...
    int actual_copy_size =
        qMin(max_dest_sz, static_cast<int>(samples_from_this_block->sample_count_per_channel() * sizeof(float)));
    ...
    for (int i = 0; i < audio_params_.channel_count(); i++) {
      ...
      memcpy(dst_ptr, samples_from_this_block->data()[i], actual_copy_size);
...

    {
      ...
      if (wave_file.open(QFile::ReadWrite)) {
        ...
        AudioRenderingParams waveform_params(SampleSummer::kSumSampleRate, audio_params_.channel_layout(),
        ...
        for (int i = 0; i < samples_from_this_block->sample_count_per_channel(); i += chunk_size) {
          QVector<SampleSummer::Sum> summary = SampleSummer::SumSamples(
              samples_from_this_block, i, qMin(chunk_size, samples_from_this_block->sample_count_per_channel() - i));
...
const AudioRenderingParams &AudioRenderWorker::audio_params() const { return audio_params_; }
