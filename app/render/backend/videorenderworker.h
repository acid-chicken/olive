...
class VideoRenderWorker : public RenderWorker {
 ...
 public:
  ...
  enum OperatingMode {
    ...
    /// Enable download (NEVER USE THIS ON ITS OWN, this is only here for checking flags, download-only mode makes no
    /// sense)
    kDownloadOnly = 0x4,
 ...
 protected:
  ...
  virtual void TextureToBuffer(const QVariant& texture, int width, int height, const QMatrix4x4& matrix, void* buffer,
                               int linesize) = 0;
  ...
  virtual void ReportUnavailableFootage(StreamPtr stream, Decoder::RetrieveState state,
                                        const rational& stream_time) override;
