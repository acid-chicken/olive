...
AudioWorker::AudioWorker(QHash<Node *, Node *> *copy_map, QObject *parent) : AudioRenderWorker(copy_map, parent) {}
...
void AudioWorker::RunNodeAccelerated(const Node *node, const TimeRange &range, NodeValueDatabase &input_params_in,
  ...
  SampleBufferPtr output_buffer =
      SampleBuffer::CreateAllocated(input_buffer->audio_params(), input_buffer->sample_count_per_channel());
  ...
  for (int i = 0; i < sample_count; i++) {
    ...
    double sample_to_second =
        static_cast<double>(sample_out_of_channel) / static_cast<double>(audio_params().sample_rate());

    rational this_sample_time = rational::fromDouble(range.in().toDouble() + sample_to_second);

    // Update all non-sample and non-footage inputs
    foreach (NodeParam *param, node->parameters()) {
      if (param->type() == NodeParam::kInput && param != sample_input) {
    ...
    node->ProcessSamples(input_params, audio_params(), input_buffer, output_buffer, i);
  }
