...
void StyleManager::ParsePaletteGroup(QSettings *ini, QPalette *palette, QPalette::ColorGroup group) {
  ...
  foreach (QString k, keys) { ParsePaletteColor(ini, palette, group, k); }
...
StyleDescriptor StyleManager::DefaultStyle() { return ListInternal().first(); }
...
void StyleManager::SetStyle(const StyleDescriptor &style) { SetStyle(style.path()); }
...
StyleDescriptor::StyleDescriptor(const QString &name, const QString &path) : name_(name), path_(path) {}

const QString &StyleDescriptor::name() const { return name_; }

const QString &StyleDescriptor::path() const { return path_; }
